...
- **빈 어노테이션 자동 정리**: title·description이 모두 빈 어노테이션은 선택이 해제되는
  시점(다른 어노테이션 선택·장면 전환·패널 닫기)에 자동 삭제한다 — 오클릭 잔재 방지.
  title 입력란 placeholder로 이 규칙을 안내한다. 소비한 번호는 재사용하지 않는다(§6.3 규칙 유지).
