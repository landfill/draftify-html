...
3. 현재 장면에 Annotation 생성: 다음 번호 자동 부여, 앵커 자동 생성(§5)
4. 요소 위에 숫자 마커 렌더, 패널의 해당 항목이 열리며 title 입력란에 포커스
5. 입력 → 500ms 디바운스로 저장 (§6.4)
```

- 요소를 조작(클릭·입력)해서 목업 상태를 바꿔야 할 때는 **미리보기 모드로 전환**해서 한다.
  "편집 모드에서 Ctrl+클릭하면 통과" 같은 예외는 만들지 않는다 (혼동 원인).
- 마커 위치는 앵커 재해석 결과 요소의 우상단. 재해석은 `MutationObserver` +
