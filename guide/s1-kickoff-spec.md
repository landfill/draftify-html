...
| DOM 캡처 | **`single-file-core`** (npm) | §7 상세. 직접 구현 금지 — CSS 인라인/폰트/이미지 처리 재발명은 S1 최대 리스크 |
...
export interface Annotation {
  ...
  markerOffset?: { dx: number; dy: number }; // 마커 표시 오프셋(px) — 기본 위치(요소 좌상단, §11 14차 개정)에서 드래그로 옮긴 상대값. §11 4차 개정
}
...
- **마커 드래그**: 편집 모드에서 마커를 드래그해 위치를 옮길 수 있다. 저장값은 절대 좌표가
  아니라 **기본 위치(요소 좌상단, §11 14차 개정) 기준 상대 오프셋**(`markerOffset`, px) — 앵커 재해석이
...
- 마커 기본 위치는 앵커 재해석 결과 요소의 좌상단 (+`markerOffset`, §11 14차 개정 — 종전 우상단). 재해석은 `MutationObserver` +
  `ResizeObserver`, 300ms 디바운스. selector 실패 시 text/attrs로 재탐색 후 selector 자동
  갱신, 재탐색도 실패하면 selector가 **유일하게** 해석되는 요소를 "위치 불확실"(점선)로
  그 위치에 렌더(서명 불일치 상태이므로 selector 갱신·저장은 하지 않음 — §11 15차 개정),
...
- 패널의 **[+ 현재 화면을 장면으로]** 버튼:
  ...
  2. 즉시 캡처 시도 (§7) → 성공 시 snapshotAsset 저장, 실패 시 §7.3
...
| T6 | SDK: 장면 등록 + 캡처 | 장면 추가 시 스냅샷 asset 업로드 확인. 스냅샷 단독으로 브라우저에서 열어 **레이아웃·구조가 원본과 동일**(폰트는 시스템 대체, 비디오·오디오는 영역만 — §11 11차) + `<script>` 0개(자동 검증) |
...
**S2 계획 입력:**
- 온보딩: S1은 경로 A(zip)만 — 실사용에서 "index.html이 없는(SSR·dev 서버 전용) 목업은 어떻게 쓰나"는 질문이 바로 나옴. **우선순위 결정(2026-07-10, 사용자): S2는 경로 B(URL 프록시) 우선.** 근거: 현실 목업 대부분이 이미 스테이징·개발 URL로 존재, SSR 목업 커버, 편집기·캡처·저장·뷰어는 무변경(온보딩 계층만 추가). 경로 D는 B 이후 실수요를 보고 판단. 비용: SSRF 방어(NFR-03)·CSP/쿠키 가공·WebSocket 엣지 (technical-spec §3.3, 리스크 R5)
