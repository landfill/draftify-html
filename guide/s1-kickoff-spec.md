...
| 2026-07-16 | 13차 개정 (용어 개정, 사용자 결정): 제품 용어 **"캡처" → "캡처"** 전면 교체 — SDK 패널·뷰어의 표시 문자열("캡처 실패 — 재시도"·"캡처 중…"·"✓ 캡처됨" 등), 사용자 문서(README·user-guide), 규범 스펙 문서(PRD·detailed-spec·technical-spec·output-standard·implementation-decisions)와 킥오프 규범 절 본문, 코드의 한국어 문자열·주석·테스트명. **코드 식별자(`freeze*`·`FreezeError`·`frozenAt`)와 역사 기록(PROGRESS 세션 로그·본 §11 기존 행)은 유지** — 10차 개정과 동일한 원칙(식별자 오염·역사 기록 왜곡 방지) | "캡처"은 기술 내부 관점의 단어로, 기획자 대상 목업 기획 서비스의 성격에 맞지 않는다(사용자 판정). "캡처"는 화면을 정지 이미지로 담는 행위의 통용어라 기획 실무자에게 즉시 이해된다. 데이터 모델의 snapshot(스냅샷)은 캡처의 결과물로 용어 위계가 자연스럽다 |
| | (구현 중 이탈 발생 시 여기에 기록) | |
