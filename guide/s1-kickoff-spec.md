...
export interface Scene {
  ...
  title: string;                 // 사용자 입력 (패널 인라인 편집. 기본값 document.title은 §11 8차에서 철회)
...
- 패널의 **[+ 현재 화면을 장면으로]** 버튼:
  1. Scene 생성 (title=빈 값 — 목록 인라인 입력으로 명명. document.title 기본값은 §11 8차에서 철회, route=현재 경로)
...
- 옵션: 스크립트 제거, CSS 인라인, 이미지 data URI화, `<canvas>`는 `toDataURL()`로
  이미지 치환. 산출물 스냅샷에 `<script>`가 하나도 없어야 한다 (뷰어에서 srcdoc로 열 때
  실행 금지 — 검증 로직으로 강제).
- **폰트는 임베드하지 않는다** (§11 11차): 웹폰트를 항상 차단하고 시스템 폰트로 렌더한다.
  기획서는 레이아웃·구조를 전달하는 문서라 폰트 시각 재현이 목적이 아니며(디폴트 폰트로 충분),
  CJK 웹폰트는 웨이트당 수 MB라 스냅샷 비대화의 최대 원인이었다.
- **비디오·오디오는 콘텐츠를 임베드하지 않고 영역(와이어프레임 박스)만 유지한다** (§11 11차):
  재생 미디어는 기획서에 불필요하고 임베드 시 수십 MB가 된다. 레이아웃 자리만 남긴다.
...
| T6 | SDK: 장면 등록 + 동결 | 장면 추가 시 스냅샷 asset 업로드 확인. 스냅샷 단독으로 브라우저에서 열어 **레이아웃·구조가 원본과 동일**(폰트는 시스템 대체, 비디오·오디오는 영역만 — §11 11차) + `<script>` 0개(자동 검증) |
...
| 2026-07-12 | 7차 개정 (전이·흐름도 착수, 로드맵 "후속 판단" → 구현 확정): 프로세스 흐름도 렌더러를 **Mermaid → 자체 경량 SVG 렌더러**로 변경. `Annotation.transition { toSceneId, condition? }`은 technical-spec §2.2 예정 필드 그대로 본 모델에 승격 (필드 추가 자유 조항) | 산출물은 단독 HTML·네트워크 0건이라 Mermaid를 쓰려면 번들 내장이 필요한데 mermaid.min.js ~3MB로 뷰어 런타임 전체(19KB)의 150배 — 모든 산출물이 상시 3MB 커진다. 장면 그래프는 수십 노드 이하 규모라 계층형 배치+화살표+간선 라벨의 자체 SVG(수 KB, 의존성 0)로 결정론적 렌더가 충분. "사람이 입력한 전이만 그린다" 원칙은 렌더러와 무관하게 유지 |
| 2026-07-12 | 8차 개정 (실사용 12차 피드백): ① 장면 제목 기본값 document.title **철회** — 빈 값으로 생성하고 패널 장면 목록에 **제목 인라인 입력** 추가 ② 동결 시 `Scene.captureWidth`/`captureHeight`(뷰포트 크기) 기록, 뷰어가 그 크기로 스냅샷을 렌더 | ① SPA는 `<title>`이 불변이라 모든 장면에 같은 무의미한 제목("tmdb-quiz" 등)이 자동으로 붙음 — 사용자가 "불필요"로 판정. 제목은 사람이 명명하는 값이고, 편집 UI가 없던 것이 결함 ② 뷰어가 스냅샷 크기를 추측하면 반응형 페이지는 좁은 iframe(기본 300px)에서 모바일 레이아웃으로, 100vh류 페이지는 scrollHeight==뷰포트라 최소 480px로 잘림(캡처의 아래쪽 소실) — 캡처 시점 뷰포트 크기가 유일하게 신뢰 가능한 기준 |
| 2026-07-12 | 9차 개정 (실사용 13차 피드백): ① 동결 후처리로 **스냅샷 이미지 최적화** — 100KB 이상 래스터(data URI png·jpeg·webp·bmp)를 WebP q0.82로 재인코딩 + 긴 변 2048px 상한 다운스케일. GIF(애니메이션)·SVG(벡터)·아이콘류(<100KB)는 제외, 이득 없으면 원본 유지, 이미지 단위 best-effort ② 경로 D 내보내기를 **chrome.downloads 직행**으로 — export 본문을 확장 메시지로 릴레이하지 않음 | ① 고해상도 이미지가 많은 페이지(영화 포스터 등)는 장면 하나가 수십 MB — 저장·공유가 실용성을 잃음. WebP 인코딩은 Chrome/Edge 전용이지만 편집 환경은 ID-02로 Chrome/Edge 한정이라 무방, 기획서 용도에 시각 열화 체감 없음 ② Chrome 확장 메시지는 64MiB 하드 리밋 — 대형 산출물이 브리지를 못 건너 내보내기가 실패(실측). 다운로드는 브라우저 네이티브 경로가 맞다 |
| 2026-07-12 | 10차 개정 (용어 개정, 사용자 결정): **사용자 표면 용어**를 "장면" → **"화면"**, "전이" → **"화면 이동"**으로 변경 — SDK 패널·뷰어·콘솔의 모든 표시 문자열과 사용자 문서(README·user-guide), PRD §2 용어표. 장면 등록 버튼은 "+ 현재 화면 등록"으로. **코드 식별자(`Scene`·`scn_`·`SCR-###`)·스키마 필드(`transition`)·스펙 문서 본문의 모델 용어("장면")는 유지** — PRD 용어표가 표면↔모델 매핑을 정의한다 | "전이"는 기획 실무 비통용어(사용자: "처음 들어본다" — 통용어는 화면 이동/연결 계열), "장면"도 실무에선 "화면"이 통용. 산출물 독자가 비개발 이해관계자인 제품이라 표면 용어는 통용어를 따른다. 코드·이력 문서까지 바꾸면 식별자 오염·역사 기록 왜곡(PROGRESS·킥오프는 불변 기록)이 더 크므로 표면만 개정 |
| 2026-07-14 | 11차 개정 (동결 크기, 사용자 결정): 동결 시 **웹폰트를 항상 차단(`blockFonts`)**하고 **비디오·오디오 콘텐츠를 임베드하지 않는다(영역만 유지)**. 비디오는 `blockVideos`(포스터+링크 대체), 오디오·비디오 포스터는 single-file 옵션에 없어 동결 직전 `neutralizeMedia`가 `src`/`srcset`/`poster`를 임시 제거한다. 종전엔 `blockFonts`가 폴백(크래시) 경로에서만 발동해 정상 성공한 페이지는 폰트를 통째 임베드했고, 오디오·포스터는 차단하지 않았다. 파생: 폰트를 안 쓰므로 `removeUnusedFonts`·`removeAlternativeFonts`(크래시 진원)를 1차에서도 끈다 → 크래시 표면도 감소. T6 DoD "시각적 동일" → "레이아웃·구조 동일(폰트 시스템 대체·미디어 영역만)" | 사용자: "기획서는 디폴트 폰트로 충분, 비디오·오디오는 최대로 봐야 섬네일이고 섬네일도 과할 수 있어 와이어프레임 영역만 잡으면 된다." 기획서는 레이아웃·의도 전달 문서라 폰트·미디어 시각 재현이 목적이 아니고, CJK 웹폰트·임베드 미디어가 스냅샷 비대화(수~수십 MB)의 최대 원인. 산출물은 단독 HTML로 공유되므로 크기 절감이 실사용 가치가 크다 |
| | (구현 중 이탈 발생 시 여기에 기록) | |
...
**S2 계획 입력:**
- 온보딩: S1은 경로 A(zip)만 — 실사용에서 "index.html이 없는(SSR·dev 서버 전용) 목업은 어떻게 쓰나"는 질문이 바로 나옴. **우선순위 결정(2026-07-10, 사용자): S2는 경로 B(URL 프록시) 우선.** 근거: 현실 목업 대부분이 이미 스테이징·개발 URL로 존재, SSR 목업 커버, 편집기·동결·저장·뷰어는 무변경(온보딩 계층만 추가). 경로 D는 B 이후 실수요를 보고 판단. 비용: SSRF 방어(NFR-03)·CSP/쿠키 가공·WebSocket 엣지 (technical-spec §3.3, 리스크 R5)
