...
4. ~~절대경로 목업 대응~~ → **v1은 온보딩 제약 문서화**("빌드를 상대 경로로/base path 맞춰서"). 인제스트
   시 절대경로 재작성은 후속. **참고**: 현재 버전은 서브도메인 서빙이라 상대·절대 둘 다 동작 — 절대경로
   깨짐은 경로 격리(D7)의 신규 제약(절대경로 목업 한정 가벼운 회귀). 빌드 가이드를 `docs/user-guide.md`에
   추가하는 것은 **현재 버전에도 유효하므로 별도 이슈로 분리 등록**(이 개편과 독립).
