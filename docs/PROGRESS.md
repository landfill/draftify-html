...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - **#99** 산출물에 대상 기기 개념이 없다 — **신규 등록 (2026-08-05, #86 논의 중 분리).** `captureWidth`가 `documentElement.clientWidth`(기획자 브라우저 창 폭)라, 모바일 화면을 기획해도 데스크톱에서 캡처하면 PC 레이아웃이 굳는다. **#86의 fit 축소는 좌우 스크롤만 없앨 뿐 증상의 절반만 덮는다.** 경로 D(확장)는 사용자의 실제 페이지 위에서 돌아 창 폭을 강제할 수 없어 방향이 갈린다. `Scene` 필드 추가는 AGENTS.md 4절 절차 필요
