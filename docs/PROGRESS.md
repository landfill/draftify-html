...
**▶ 다음 세션 시작점 (2026-08-06 재갱신) — **#99 착수됨. 조사·비용 정리가 끝났고 사용자 결정 4건 대기다.** 코드는 0줄이고, 결정이 나오면 `s1-kickoff-spec.md` 11절 21차 개정부터 쓴다(AGENTS.md 4절). **이슈 본문의 전제 두 개가 실측으로 뒤집혔으니 본문만 읽고 시작하지 말 것** — 정리는 [#99 코멘트](https://github.com/landfill/draftify-html/issues/99#issuecomment-5197244021)와 맨 아래 세션 로그에 있다. 아래는 그 앞의 기록이다: **#90 종결.** 스크린샷 4장을 2x로 재촬영해 `main`에 반영했고(`79ae30e`·`e5d111c`), 이슈 본문의 옛 진단도 정정한 뒤 닫았다. 그 앞의 #97은 C안(수동 실행 규약화)으로 종결됐고(merge `7a0196b`), T87-14로 **네 표면이 모두 `THEME_TOKENS`를 쓴다**(merge `54ece29`). **다음 세션은 아래 "열린 이슈"에서 골라 시작하면 된다** — 우선순위는 `#99`(대상 기기) → `#57`(마이그레이션 통합. **끝나면 #97 A안 재검토 조건이 된다**). UI를 건드리기 전에 `docs/ui-standard.md` **3.0절(기각 이력)** 과 **6.5절(포커스 링 함정)** 을 반드시 읽는다. **스크린샷을 다시 찍을 일이 생기면 아래 세션 로그의 `deviceScaleFactor: 2` 경고를 먼저 읽는다.**
...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - **#99** 산출물에 대상 기기 개념이 없다 — **착수·조사 완료, 사용자 결정 4건 대기 (2026-08-06).** 세 방향 비용 정리를 [이슈 코멘트](https://github.com/landfill/draftify-html/issues/99#issuecomment-5197244021)에 등록했다. **본문 전제 두 개가 실측으로 뒤집혔다**: ① A·B도 iframe이 아니라 새 탭 최상위라 **지금은 어느 경로도 폭 강제가 안 된다** ② 캡처가 브레이크포인트를 지워(`removeUnusedStyles`·`removeHiddenElements`) **사후에 폭만 줄이면 모바일이 아니라 깨진 데스크톱이 된다**(실측: `column` 전환 0건). 3안은 1안의 부분집합이라 **택일이 아니라 누적 3단계**다. 아래는 등록 당시 기록이다: **신규 등록 (2026-08-05, #86 논의 중 분리).** `captureWidth`가 `documentElement.clientWidth`(기획자 브라우저 창 폭)라, 모바일 화면을 기획해도 데스크톱에서 캡처하면 PC 레이아웃이 굳는다. **#86의 fit 축소는 좌우 스크롤만 없앨 뿐 증상의 절반만 덮는다.** 경로 D(확장)는 사용자의 실제 페이지 위에서 돌아 창 폭을 강제할 수 없어 방향이 갈린다. `Scene` 필드 추가는 AGENTS.md 4절 절차 필요
...
다음 할 일:
- **사용자 결정 4건 대기** — ① 1단계만 vs 1+2 동시 ② `targetDevice` 값 형태(프리셋 390/834/1440 vs 자유 px) ③ 적용 단위(장면별 vs 프로젝트 기본값+오버라이드) ④ 뷰어 렌더 정책(**대상 기기 폭으로 렌더하면 위 근거 ②의 깨진 화면이 그대로 산출물이 된다** — 프레임·경고만 대상 기기로 하고 렌더 폭은 `captureWidth` 유지가 안전한지)
- ①~③이 정해지면 `s1-kickoff-spec.md` 11절 **21차 개정**부터 쓴다 (AGENTS.md 4절 — 코드보다 스펙이 먼저)

막힌 지점: **위 결정 4건.** 특히 ①은 비용 차가 한 자릿수 배다.
