...
- 기존 데이터 이관(§9-3) 결정: **이관 없음, 빈 DB로 시작.** 기존 data/는 내부 시연·설명용 폐기 대상 — 새 Supabase로 안 옮김, 이관 스크립트 불필요. 상류 결정(D1~D8 + 인증·권한·데이터) 전부 확정, 남은 건 저강도 4건(절대경로 목업·경로D CORS·쿼터/레이트리밋·브랜치 전략)뿐.
- 다음 할 일: 저강도 4건 정리 후 RFC → 킥오프 스펙 승격(PRD §7.1·NFR-01·§7.2 동기화, WBS를 technical-spec §9.2로). PR #35 병합은 사용자 동의 대기.
- 막힌 지점: 없음.
