...
**▶ 다음 세션 시작점 (2026-08-05 갱신) — **#86 종결(PR #98 merge `8dcb9b7`, 프로덕션 반영 확인). 진행 중인 작업이 없다.** 산출물이 화면기획서 2컬럼(타이틀 + 화면영역 + 디스크립션)이 됐다. **다음 세션은 아래 "열린 이슈"에서 골라 시작하면 된다** — 우선순위는 `#90`(README 스크린샷. **#86으로 산출물 컷까지 노후화됐으니 5장을 한 번에 찍는다**) → `#97`(공개판 E2E의 CI 공백. 선택지 3안 중 **택일이 제품 결정**이라 사용자 판단이 선행한다) → `T87-14`(산출물 시각 언어. **#86이 끝나 착수 조건이 충족됐다** — `ui-standard.md` 0절) → `#99`(대상 기기). UI를 건드리기 전에 `docs/ui-standard.md` **3.0절(기각 이력)** 과 **6.5절(포커스 링 함정)** 을 반드시 읽는다.**
...
> **⚠️ 공개판 E2E(`test:e2e:web`)는 CI에서 돌지 않는다 (#97에서 확인).** `.github/workflows/ci.yml:45`는
> `npm run test:e2e`(사내판)만 실행한다. **공개판을 건드리는 작업은 `npm run test:e2e:web`을 로컬에서
> 직접 돌려야 한다** — 안 돌리면 아무도 안 돌린 것이다. #91이 정확히 그 공백으로 PR #88부터
> 하루 넘게 살아남았다. `apps/web/.env.local`이 없으면 8본이 **조용히 전부 스킵**되므로,
> 통과 개수(8 passed)를 눈으로 확인한다.
...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - **#97** 공개판 E2E가 CI에서 한 번도 실행되지 않는다 — **신규 등록 (2026-08-05, #91 규명 중 발견).** `main` 병합 = 프로덕션인 쪽에 자동 회귀 그물이 없다. **한 줄 추가로 끝나지 않는다**: 키가 없으면 8본이 조용히 전부 스킵되고, 키를 넣으면 Preview·Production이 공유하는 **프로덕션 DB에 푸시마다 테스트 사용자·프로젝트가 쌓인다.** 선택지 A(테스트 전용 Supabase·권고) / B(프로덕션 DB + 정리 강제) / C(수동 실행 규약) 중 **택일이 제품 결정.** A는 #57과 맞물린다(먼저 하면 비용이 내려간다)
...
- [—] T87-14 산출물 — **이 이슈에서 하지 않기로 정하고 #87을 닫았다** (`ui-standard.md` 0절·6.2절). 착수 조건은 **#86 완료 + AGENTS.md 4절 절차**이고, 그때는 #86의 후속으로 다룬다 — 이 체크리스트로 돌아오지 않는다
