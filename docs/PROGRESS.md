...
**▶ 다음 세션 시작점 (2026-08-06 갱신) — **`#90` 재촬영 완료, 브랜치 `docs/readme-screenshots-90`이 병합 대기 중이다(사용자 동의 필요 — AGENTS.md 6절).** 무변경은 확장 팝업 하나뿐이었고 나머지 4장을 새로 찍었다. **이슈 #90 본문은 아직 옛 진단 그대로다 — 사용자 확인 후 정정한다.** 그 앞의 #97은 C안(수동 실행 규약화)으로 종결됐고(merge `7a0196b`), T87-14로 **네 표면이 모두 `THEME_TOKENS`를 쓴다**(merge `54ece29`). **병합 뒤 다음 순서는** `#99`(대상 기기) → `#57`(마이그레이션 통합. **끝나면 #97 A안 재검토 조건이 된다**). UI를 건드리기 전에 `docs/ui-standard.md` **3.0절(기각 이력)** 과 **6.5절(포커스 링 함정)** 을 반드시 읽는다.**
...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - **#90** README 경로 D 스크린샷 노후화 — **재촬영 완료, 브랜치 `docs/readme-screenshots-90` (2026-08-06, 병합 대기).** 무변경은 2번째(확장 팝업) 하나뿐이었고 1·3·4·5를 새로 찍었다. 등록 당시 진단의 어긋남은 위 "디자인 이슈 진행 상태" 참조
  ...
  - **#12 · #13** `[실수요 대기]` 유지
  - 종결: **#97(PR #100)**, **#87 T87-14(PR #101)**, **#86(PR #98)**, **#91(PR #96)**, **#93(PR #95)**, **#87(PR #92·#94)**, **#85(PR #88)**, **#81(PR #84)**, **#82(PR #83)**, **#71(PR #80)**, #64·#70(PR #79), #77(PR #78), #65(PR #76), #68(PR #75), #63(PR #72), #43(PR #67), #47(PR #58), #50·#51(PR #53), #34·#36·#42(이전 세션)
...
다음 할 일:
- 브랜치 `docs/readme-screenshots-90` **병합 대기 — 사용자 동의 필요** (AGENTS.md 6절)
- **이슈 #90 본문 정정** — 1번째를 "유효"로 내리고 4·5번째를 노후로 승격 (사용자 확인 후)
- 병합 후 `#99`(대상 기기) → `#57`(마이그레이션 통합)
