...
**▶ 다음 세션 시작점 (2026-07-30 갱신) — 배포 운영 단계. 다음 작업은 이슈 #45(A안).**

> **핸드오프 요약 (이 줄부터 5줄만 읽어도 이어받을 수 있게)**
> 1. `main`이 배포 브랜치다. **머지 = 프로덕션 반영.** 작업은 `main`에서 딴 토픽 브랜치 → PR → 머지.
> 2. **다음 작업 = #45(A안만)**. #47은 구현·검증 완료(브랜치 `fix/token-reissue-atomic`, 병합 대기). **#47의 UNIQUE 마이그레이션은 이미 프로덕션 Supabase에 적용된 상태**다 — 코드 병합 전이라도 기존 delete+insert 코드는 정상 동작한다(경합 시에만 에러).
...
- **다음 할 일 — 우선순위 재정렬 (2026-07-30, 사용자 결정).** 이전 기록은 `#43 → #45`였다. **화면에 직접 보이는 것을 먼저** 처리한다는 사용자 판단으로 #50·#51을 선두에 두고, 그 뒤를 실피해·비용 순으로 잇는다:
  ...
  3. ~~**#47 토큰 재발급 경합**~~ — **구현·검증 완료 (브랜치 `fix/token-reissue-atomic`, 병합 대기).** UNIQUE 제약 마이그레이션은 **프로덕션에 이미 적용**했다(중복 0건 확인 후). 상세는 맨 위 세션 로그. **다음에 마이그레이션이 딸린 작업을 할 때 재사용할 교훈: 코드가 제약에 의존하면(upsert의 `onConflict` 등) 마이그레이션이 먼저다** — 반대로 하면 그 기능이 프로덕션에서 즉시 죽는다. #45는 성질이 반대(권한 회수 → 코드 먼저).
  4. **▶▶ #45 남용 방어 우회 — 여기서 시작한다.** 선택지 **A만 먼저**(projects INSERT revoke → 서버 경유). B(Storage)는 D5·D6과 얽혀 킥오프 §4 절차 필요하니 분리. 실사용자가 소유자 1명뿐이라 당장의 실피해는 낮지만 사용자가 늘기 전에 닫는다. **적용 순서 주의: 코드(서버 경유)가 먼저, RLS를 좁히는 마이그레이션이 나중**
  5. **#43 확장 배포 경로** — B안(zip 다운로드)만. A(웹스토어)는 심사·비용·개인정보 페이지가 딸려오니 별건. 착수 전 **Preview 환경변수 3종** 확인(아래)
...
- **열린 이슈 (8건)** — 처리 순서는 위 "다음 할 일":
  ...
  - **#47** 토큰 재발급 비원자성 — **구현 완료·PR 병합 시 종결** (`fix/token-reissue-atomic`)
  - **#45** 브라우저 직접 DB·Storage 쓰기가 쿼터·레이트리밋 우회 (A안만 먼저) ← **다음 작업**
  - **#43** 공개판 확장 배포 경로 (B안=zip 다운로드만. 착수 전 Preview 환경변수 3종 확인)
  - **#46** 매직링크 크로스디바이스 — **⛔ 대기(SMTP 제약, 사용자 결정 필요)**
  - **#48** 목업 오리진 격리 — **조건 대기**(외부 공유를 열 때가 조건. 미리 하면 CORS 전환 비용을 쓸 곳 없이 지불)
  - **#12 · #13** `[실수요 대기]` 유지
  - 종결: #50·#51(PR #53), #34·#36·#42(이전 세션)
...
### 2026-07-30 — #47 토큰 재발급 원자성 (UNIQUE 제약 + upsert) — **마이그레이션 프로덕션 적용 완료**
