...
**▶ 다음 세션 시작점 (2026-07-30 갱신) — 배포 운영 단계. 다음 작업은 이슈 #47.**

> **핸드오프 요약 (이 줄부터 5줄만 읽어도 이어받을 수 있게)**
> 1. `main`이 배포 브랜치다. **머지 = 프로덕션 반영.** 작업은 `main`에서 딴 토픽 브랜치 → PR → 머지.
> 2. **다음 작업 = #47** (토큰 재발급 비원자성). 마이그레이션이 포함되고 **Supabase는 하나뿐**이라 적용 즉시 프로덕션에 걸린다 — 아래 §착수 절차를 반드시 따를 것.
...
- **다음 할 일 — 우선순위 재정렬 (2026-07-30, 사용자 결정).** 이전 기록은 `#43 → #45`였다. **화면에 직접 보이는 것을 먼저** 처리한다는 사용자 판단으로 #50·#51을 선두에 두고, 그 뒤를 실피해·비용 순으로 잇는다:
  ...
  3. **▶▶ #47 토큰 재발급 경합 — 여기서 시작한다.** 걸리면 프로젝트가 인증 불능으로 잠긴다. #42로 [토큰 재발급] 버튼이 생겨 더블클릭만으로 재현 가능해졌다.
     - **무엇**: `apps/web/lib/store/tokenStore.ts`의 재발급이 `revokeToken()`(DELETE) → `issueToken()`(INSERT) 2단계이고 원자적이지 않다. `project_id`에 UNIQUE가 없어 경합 시 행이 2개 남고, `verifyToken()`의 `maybeSingle()`이 에러를 내 **새 토큰 둘 다 무용지물**이 된다.
     - **어떻게**: ① `project_tokens.project_id` UNIQUE 제약 마이그레이션 ② delete+insert → `upsert({ onConflict: 'project_id' })`
     - **⚠️ 착수 절차 (순서 지킬 것)**: Supabase는 **Preview·Production 공용 1개**다. UNIQUE 제약은 **기존 중복 행이 있으면 적용 자체가 실패**한다 → (a) 먼저 중복 조회(`select project_id, count(*) ... group by having count(*) > 1`) (b) 있으면 정리(토큰은 해시만 저장하므로 지워도 재발급으로 복구된다) (c) 그 다음 마이그레이션 적용. 코드와 마이그레이션의 적용 순서도 함께 고려할 것(#45 항목의 주의와 같은 성질).
  4. **#45 남용 방어 우회** — 선택지 **A만 먼저**(projects INSERT revoke → 서버 경유). B(Storage)는 D5·D6과 얽혀 킥오프 §4 절차 필요하니 분리. 실사용자가 소유자 1명뿐이라 당장의 실피해는 낮지만 사용자가 늘기 전에 닫는다
...
- **열린 이슈 (7건)** — 처리 순서는 위 "다음 할 일":
  - **#47** 토큰 재발급 비원자성 ← **다음 작업**
  - **#45** 브라우저 직접 DB·Storage 쓰기가 쿼터·레이트리밋 우회 (A안만 먼저)
