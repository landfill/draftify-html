...
**▶ 다음 세션 시작점 (2026-08-06 갱신) — **PR 두 건이 열려 있고 둘 다 병합 대기다(사용자 동의 필요). 봇 리뷰는 전부 처리했다.** ① **PR #100**(`docs/e2e-web-manual-rule-97`) — #97을 **C안(수동 실행 규약화)**으로 확정해 AGENTS.md 6절에 명문화. 리뷰 4건 처리(3건 반영·1건 기각). ② **PR #101**(`feat/output-visual-language-t87-14`) — **T87-14 산출물 시각 언어**. 킥오프 20차 → 문서 동기화 → 구현 → 리뷰 반영 4커밋, CI green. **다음 세션은 두 PR의 남은 봇 리뷰를 확인하고 병합 여부를 사용자에게 확인하는 것부터 시작한다.** 그 뒤 남은 우선순위는 `#90`(README 스크린샷 5장 — **T87-14가 병합되면 산출물 컷이 또 바뀌므로 병합 후에 찍는다**) → `#99`(대상 기기). UI를 건드리기 전에 `docs/ui-standard.md` **3.0절(기각 이력)** 과 **6.5절(포커스 링 함정)** 을 반드시 읽는다.**
...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - **#97** 공개판 E2E가 CI에서 한 번도 실행되지 않는다 — **C안(수동 실행 규약화)으로 결정 (사용자, 2026-08-06). PR #100이 AGENTS.md 6절에 명문화하며 `Closes #97`. 병합 대기.** A안(테스트 전용 Supabase)은 **#57이 끝난 뒤 재검토**한다 — 마이그레이션 이중 적용 비용이 그때 내려간다. 아래는 등록 당시 기록이다: **신규 등록 (2026-08-05, #91 규명 중 발견).** `main` 병합 = 프로덕션인 쪽에 자동 회귀 그물이 없다. **한 줄 추가로 끝나지 않는다**: 키가 없으면 8본이 조용히 전부 스킵되고, 키를 넣으면 Preview·Production이 공유하는 **프로덕션 DB에 푸시마다 테스트 사용자·프로젝트가 쌓인다.** 선택지 A(테스트 전용 Supabase·권고) / B(프로덕션 DB + 정리 강제) / C(수동 실행 규약) 중 **택일이 제품 결정.** A는 #57과 맞물린다(먼저 하면 비용이 내려간다)
  ...
  - **#12 · #13** `[실수요 대기]` 유지
  - 종결: **#86(PR #98)**, **#91(PR #96)**, **#93(PR #95)**, **#87(PR #92·#94)**, **#85(PR #88)**, **#81(PR #84)**, **#82(PR #83)**, **#71(PR #80)**, #64·#70(PR #79), #77(PR #78), #65(PR #76), #68(PR #75), #63(PR #72), #43(PR #67), #47(PR #58), #50·#51(PR #53), #34·#36·#42(이전 세션)
...
- [x] T87-14 산출물 — **#87을 닫을 때는 범위 밖이었으나(착수 조건 = #86 완료 + AGENTS.md 4절 절차), #86이 19차로 끝나 조건이 충족돼 2026-08-06에 따로 처리했다.** 킥오프 **20차** → 문서 동기화 → 구현. 강조색·중립 회색·헤더 그라디언트·반경·포커스 링을 한 번에 정리해 **네 표면이 모두 `THEME_TOKENS`를 쓴다.** 상세는 `ui-standard.md` 6.8절. **PR #101 — 병합 대기**
