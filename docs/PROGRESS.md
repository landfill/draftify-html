...
- [x] T87-6 로그인 화면 — 13px `<h1>`·버튼 위계·인라인 style 제거·서비스 설명
- [x] T87-7 랜딩 히어로 중앙 정렬 — CSS만, 마크업 무변경
...
- **로그인** (`fix:`) — ① `<h1>`이 `.c-section-title` 재사용으로 13px이던 것을 `.c-login-title` 22px로 분리 ② Google/매직링크가 둘 다 채움 강조색이라 주 동작이 없던 것을 `.c-btn-social` 테두리형으로 분리 ③ **인라인 `style` 4곳 제거** — 콘솔 2단 폼 그리드를 이 화면에서만 되돌리던 땜질을 `.c-login-card .c-row` CSS로 이관 ④ 서비스 설명 1줄 추가
