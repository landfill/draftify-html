...
**▶ 다음 세션 시작점 (2026-08-06 갱신) — **PR #100·#101 병합 완료. 진행 중인 작업이 없다.** #97은 C안(수동 실행 규약화)으로 종결됐고(merge `7a0196b`), T87-14로 **네 표면이 모두 `THEME_TOKENS`를 쓴다**(merge `54ece29`). **다음 세션은 아래 "열린 이슈"에서 골라 시작하면 된다** — 우선순위는 `#90`(README 스크린샷. **#86·T87-14로 산출물 컷까지 노후화됐으니 5장을 한 번에 찍는다.** 이제 산출물을 더 바꿀 예정이 없어 다시 찍을 일이 없다) → `#99`(대상 기기) → `#57`(마이그레이션 통합. **끝나면 #97 A안 재검토 조건이 된다**). UI를 건드리기 전에 `docs/ui-standard.md` **3.0절(기각 이력)** 과 **6.5절(포커스 링 함정)** 을 반드시 읽는다.**
...
- **#90 README 스크린샷 노후화** — 신규 등록 (2026-08-02). 경로 D 절의 1·3번째(콘솔 화면)가 #43·#77·#82·#85·T87-10 이후 실제 화면과 다르다. 2·5번째는 유효하고, **4번째(SDK 패널)는 T87-13에서 팔레트가 바뀌었으므로 이제 다시 찍어야 한다**
...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - **#90** README 경로 D 스크린샷 노후화 — 1·3번째(콘솔 화면)가 #43·#77·#82·#85·T87-10 이후 실제와 다르다. **4번째(SDK 패널)도 T87-13에서 팔레트가 바뀌었으니 같이 찍는다**
