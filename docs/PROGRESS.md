...
**▶ 다음 세션 시작점 (2026-07-31 갱신) — #43 프로덕션 반영 완료. 다음 작업은 사용자 우선순위 지정 대기.**

> **핸드오프 요약 (이 줄부터 5줄만 읽어도 이어받을 수 있게)**
> 1. `main`이 배포 브랜치다. **머지 = 프로덕션 반영.** 작업은 `main`에서 딴 토픽 브랜치 → PR → 머지.
> 2. **#43 완료·종결 (PR #67, merge `03a3101`).** 공개판에서 `/download/mockspec-extension.zip`을 받아 개발자 모드로 설치하는 흐름이 프로덕션에 있다. **단 하나 미검증: 실제 Chrome에 unpacked 로드하는 마지막 한 걸음** — Preview가 Vercel SSO로 막혀 있어 자동 검증이 불가능했고, ZIP 구조·manifest·CSP는 실 산출물을 풀어 정적 검증했다. **프로덕션에서 1회 수동 확인 필요.** #45는 **2번(Storage 직접 업로드)이 남아 이슈가 열려 있다**.
...
- [x] T43-6 배포 검증 — **PR #67 병합 완료 (merge `03a3101`), 이슈 #43 종결.** 봇 리뷰 7건 처리(3건 수정·2건 철회·2건 중복), 리뷰 스레드 7개 전부 resolved, CI·Vercel 전부 pass.
  - **⚠️ 단 하나 남은 미검증 = 실 Chrome unpacked 로드.** Preview가 **Vercel SSO로 보호돼 자동 검증이 불가능**했고(사용자 확인), `chrome://extensions`는 브라우저 자동화로 접근할 수 없다. 대신 실 배포 ZIP을 압축 해제해 정적 검증했다: 최상위 `mockspec-extension/` 폴더 1개, 필수 6파일, MV3 manifest가 참조하는 4파일 실재, `popup.html` 인라인 스크립트 0건(CSP 위반 없음), 타임스탬프 전부 `1980-01-01`(결정적 빌드). **Chrome이 거부할 정적 사유는 없으나, 프로덕션에서 1회 수동 설치 확인이 남았다.**
