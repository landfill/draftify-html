...
**▶ 다음 세션 시작점 (2026-08-02 갱신) — T87-10(F-01 `--c-faint` 대비) 구현 완료, 브랜치 `fix/faint-token-merge-87`에 **미커밋 상태**로 있다. 다음 착수는 T87-11(F-02 토큰 단일 원천). UI를 건드리기 전에 `docs/ui-standard.md` 3.0절(기각 이력)을 반드시 읽는다.**
...
- [ ] T87-11 F-02 토큰 단일 원천 — `shared`에 `THEME_TOKENS` + `globals.css` 파리티 vitest
