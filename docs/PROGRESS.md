...
- **다음 할 일 — 우선순위 재정렬 (2026-07-30).** 이전 기록은 `#43 → #45`였으나, 그 뒤 등록된 #46·#47이 **값싸고 실피해가 확실**해서 앞으로 당겼다:
  1. **#46 매직링크 크로스디바이스** — 지금 로그인 퍼널이 깨져 있다(노트북 요청 → 휴대폰 열기 실패). 코드 0줄, Supabase 이메일 템플릿 한 줄. 실검증은 다른 기기에서 수동으로 해야 한다
  2. **#47 토큰 재발급 경합** — 걸리면 프로젝트가 인증 불능으로 잠긴다. #42로 버튼이 생겨 더블클릭만으로 재현 가능. `project_tokens.project_id` UNIQUE + upsert
  3. **#45 남용 방어 우회** — 선택지 **A만 먼저**(projects INSERT revoke → 서버 경유). B(Storage)는 D5·D6과 얽혀 킥오프 §4 절차 필요하니 분리. 실사용자가 소유자 1명뿐이라 당장의 실피해는 낮지만 사용자가 늘기 전에 닫는다
  4. **#43 확장 배포 경로** — B안(zip 다운로드)만. A(웹스토어)는 심사·비용·개인정보 페이지가 딸려오니 별건. 착수 전 **Preview 환경변수 3종** 확인(아래)
  5. **#50 + #51 한 PR로** — 둘 다 `apps/web` 콘솔 UI이고 `shell-header.tsx`·`console-home.tsx`에서 파일이 겹친다
...
- 다음 할 일: **#46 매직링크 크로스디바이스** → #47 → #45(A안) → #43 → #50+#51(한 PR). 위 "현재 단계"의 우선순위 근거 참조.
