...
- **⚠️ 브랜치 운영이 바뀌었다 (2026-07-29): Vercel Production Branch = `main`.**
  ...
  - **`open-service` 브랜치는 역할이 끝났다.** §6대로 삭제하지 않고 남겨 두되 더 이상 쓰지 않는다(`main`과 동일 커밋 `68f06e3`에서 정지). 워크트리 `../Draftify-open-service`도 정리 대상 — 지우기 전에 **`apps/web/.env.local`을 main 워크트리로 옮겨야 한다**(gitignore라 커밋에 없다).
...
- **다음 할 일: 이슈 #43 — 공개판 사용자용 확장 배포 경로.** 경로 D 진입점은 #42로 콘솔에 생겼지만 공개 사용자는 확장 자체를 받을 방법이 없다(현 안내가 저장소 clone·빌드 전제). 선택지 A(웹스토어)·B(사이트에서 zip 다운로드 + 개발자 모드 안내)와 구현 고려사항(`vercel-build`에서 확장 dist를 zip으로 묶어 `apps/web/public/` 서빙, manifest `version` 갱신 규칙, "확장 설치 → 프로젝트 생성" 순서로 안내 재배치)은 이슈에 정리돼 있다.
...
- 열린 이슈: **#45(보안·우선 — 브라우저 직접 DB·Storage 쓰기가 쿼터·레이트리밋 우회)** · #43(확장 배포 경로, **내일 작업**) · #46(매직링크 크로스 디바이스 — 이메일 템플릿 설정) · #47(토큰 재발급 비원자성) · #42 잔여(snippet 콘솔 내보내기) · #13·#12(실수요 대기)
...
- 완료: **이슈 #48 개설** — 작업 항목 9개(usercontent 프로젝트 신설 / 프로젝트 단위 단기 capability 인증 배관 / SDK 저장 CORS+토큰 전환 / `vercel.json` CORS / 확장 `host_permissions` 호스트 1개 추가 / `<base>`·SPA fallback 검증 / §7.4 부작용 해소 확인 / 두 오리진 E2E)와 완료 조건 포함. 문서 3곳에 교차 참조: `guide/open-service-rfc.md` §7.3·§9-8, `guide/open-service-kickoff-spec.md` §7.3·§9 + §11 이력 행.
