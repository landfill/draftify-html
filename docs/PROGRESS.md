...
**▶ 다음 세션 시작점 (2026-08-06 갱신) — **PR 두 건이 열려 있고 둘 다 병합 대기다(사용자 동의 필요).** ① **PR #100**(`docs/e2e-web-manual-rule-97`) — #97을 **C안(수동 실행 규약화)**으로 확정해 AGENTS.md 6절에 명문화. 봇 리뷰 3건 처리 완료(2건 반영·1건 기각). ② **PR #101**(`feat/output-visual-language-t87-14`) — **T87-14 산출물 시각 언어**. 킥오프 20차 → 문서 동기화 → 구현 3커밋, CI green. **다음 세션은 두 PR의 남은 봇 리뷰를 확인하고 병합 여부를 사용자에게 확인하는 것부터 시작한다.** 그 뒤 남은 우선순위는 `#90`(README 스크린샷 5장 — **T87-14가 병합되면 산출물 컷이 또 바뀌므로 병합 후에 찍는다**) → `#99`(대상 기기). UI를 건드리기 전에 `docs/ui-standard.md` **3.0절(기각 이력)** 과 **6.5절(포커스 링 함정)** 을 반드시 읽는다.**
...
> 규약 본문은 **AGENTS.md 6절 "병합 전 검증"**(PR #100 병합 후 유효).
