...
**▶ 다음 세션 시작점 (2026-08-02 갱신) — **#87에서 할 일이 전부 끝났고 [PR #92](https://github.com/landfill/draftify-html/pull/92)가 열려 있다** (T87-10·11·12·13). 작업하며 브랜치를 4개로 쌓았지만 **사용자 요청으로 하나(`fix/ui-standard-remaining-87`, 10커밋)로 합쳤다** — PR #89 때와 같은 방식이고, 커밋 단위로는 성격이 그대로 나뉘어 있어 커밋별로 리뷰하면 된다. **다음 할 일은 PR #92의 리뷰 대응이고, 병합되면 `Closes #87`로 이슈가 닫힌다.** 병합은 건별 사용자 동의가 필요하다(AGENTS.md 6절). T87-14(산출물)는 #86 종속이라 이 이슈에서 하지 않는다. UI를 건드리기 전에 `docs/ui-standard.md` 3.0절(기각 이력)을 반드시 읽는다.**

> **⚠️ 이 PR의 리뷰 종료 조건을 먼저 정해 뒀다** (PR #89의 교훈 — 아래 2026-08-02 로그 "병합 후 리뷰 루프에서 배운 것"). `ui-standard.md`에 이번에 절이 4개(6.3~6.6) 늘어, 산문 P2가 라운드마다 새로 나올 수 있다. **코드 파일 지적과 P1만 반영하고, 문서 P2는 별건으로 돌린다.**

**디자인 이슈 진행 상태 (2026-08-02):**

- **#87 전체 UI 스타일** — **PR #89 main 병합 완료 (merge `56b7735`, 2026-08-02). 이슈는 계속 열려 있다** — 남은 것은 T87-11~14 (**T87-10은 2026-08-02 구현 완료**).
- **#90 README 스크린샷 노후화** — 신규 등록 (2026-08-02). 경로 D 절의 1·3번째(콘솔 화면)가 #43·#77·#82·#85·T87-10 이후 실제 화면과 다르다. 2·4·5번째는 유효하되 **4번째(SDK 패널)는 T87-13에서 팔레트가 바뀔 때 같이 찍는다** 브랜치는 **하나였다**(사용자 요청으로 통합). 커밋만 성격별로 나눠 뒀다: `docs:`(규범 문서) → `fix:`(로그인) → `feat:`(히어로 중앙 정렬) → `docs:`(이 PROGRESS) → 리뷰 반영 3커밋. **⚠️ A/B 검증에서 "본문 글자를 키우는" 방향이 세 번 기각됐다** — 콘솔·가이드·FAQ·랜딩 프리뷰는 현재 유지다. 채택된 것은 구조적 결함을 고친 경우뿐. 상세는 `docs/ui-standard.md` 3.0절
