...
**▶ 다음 세션 시작점 (2026-08-01 갱신) — #71(함수 리전 `icn1`, PR #80)·#82(헤더 [내 프로젝트] 구분, PR #83) **둘 다 완료·종결·프로덕션 반영**. 콘솔 홈 목록 표시가 **7.1초 → 0.92초**. 다음 작업은 사용자 우선순위 지정 대기 — 후보는 **#46**(매직링크, 코드 0줄·결정만 하면 풀린다)·**#81**(RSC 프리페치).**
...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - **#81** 콘솔 홈 목록 RSC 프리페치 (#71 후속) — 목록 표시 **924ms 중 577ms가 클라이언트 워터폴**이다(HTML 347ms 완료 → 77ms 공백 → API 500ms). 서버에서 미리 실어 보내면 이 왕복이 사라진다. **최소 변경 원칙**: `loadProjects()`와 `GET /api/projects`는 남긴다(업로드·삭제·토큰 발급 후 갱신이 쓴다). N+1·`getUser()` 중복·`spec` 전체 전송도 이 이슈에 함께 적었지만 **각각 수십 ms라 단독 착수 값어치는 없다**(리전 이동으로 왕복 단가가 ~170ms → ~17ms)
  ...
  - **#12 · #13** `[실수요 대기]` 유지
  - 종결: **#82(PR #83)**, **#71(PR #80)**, #64·#70(PR #79), #77(PR #78), #65(PR #76), #68(PR #75), #63(PR #72), #43(PR #67), #47(PR #58), #50·#51(PR #53), #34·#36·#42(이전 세션)
