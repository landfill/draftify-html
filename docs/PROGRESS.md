...
- **다음 할 일 — 우선순위 재정렬 (2026-07-30, 사용자 결정).** 이전 기록은 `#43 → #45`였다. **화면에 직접 보이는 것을 먼저** 처리한다는 사용자 판단으로 #50·#51을 선두에 두고, 그 뒤를 실피해·비용 순으로 잇는다:
  1. **#50 + #51 한 PR로** — 둘 다 `apps/web` 콘솔 UI이고 `shell-header.tsx`·`console-home.tsx`에서 파일이 겹친다. #50은 표시용 `DISPLAY_NAME` 신설(`WORKING_NAME`은 식별자 겸용이라 불변), #51은 공개판 "새 프로젝트"를 **2탭**으로 재구성(`URL 등록`은 D2로 제외)
  ...
  3. **#47 토큰 재발급 경합** — 걸리면 프로젝트가 인증 불능으로 잠긴다. #42로 버튼이 생겨 더블클릭만으로 재현 가능. `project_tokens.project_id` UNIQUE + upsert
...
- 열린 이슈 (9건): #46(매직링크 크로스디바이스) · #47(토큰 재발급 비원자성) · #45(브라우저 직접 DB·Storage 쓰기가 쿼터·레이트리밋 우회) · #43(확장 배포 경로) · #50(타이틀 구분 — `MockSpec Local` / `MockSpec Cloud`) · #51(공개판 콘솔 탭 재구성) · #48(오리진 격리 — 외부 공유 선행, 조건 대기) · #13·#12(실수요 대기). 처리 순서는 위 "다음 할 일" 참조
