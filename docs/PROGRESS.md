...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - **#103** 편집 직후 내보내면 마지막 편집이 산출물에 빠진다 — **신규 등록 (2026-08-06, PR #102 Codex 지적에서 분리).** 저장은 500ms 디바운스 + 전체 PUT인데 **내보내기는 서버가 마지막 저장본을 읽고**(로컬 `doc`은 안 실린다) 버튼은 `saving` 중에도 눌린다. 셋이 겹쳐 **오류도 경고도 없이 옛 값이 담긴다** — 조용한 실패라 더 나쁘다. `targetDevice`에서 발견됐지만 화면 제목·경로·어노테이션·전이·섹션 라벨 **전부**가 같은 경로다. 수정은 ① `runExport` 진입 시 flush ② `saveStatus !== "saved"`면 버튼 차단 중 택일이고 **2안이 싸지만 오프라인 큐(`offline`) 상태 처리를 함께 정해야 한다.** 어느 쪽이든 내보내기 가능 조건은 킥오프 §11 6차 소관이라 **스펙 개정이 선행**한다(AGENTS.md 4절)
  ...
  - **#12 · #13** `[실수요 대기]` 유지
  - 종결: **#99(PR #102)**, **#90(main 직접 `79ae30e`·`e5d111c`)**, **#97(PR #100)**, **#87 T87-14(PR #101)**, **#86(PR #98)**, **#91(PR #96)**, **#93(PR #95)**, **#87(PR #92·#94)**, **#85(PR #88)**, **#81(PR #84)**, **#82(PR #83)**, **#71(PR #80)**, #64·#70(PR #79), #77(PR #78), #65(PR #76), #68(PR #75), #63(PR #72), #43(PR #67), #47(PR #58), #50·#51(PR #53), #34·#36·#42(이전 세션)
