...
**▶ 다음 세션 시작점 (2026-08-01 갱신) — #85 공개판 메인 화면 구현 완료·PR #88 검증/리뷰 대기. 비로그인 `/`은 서비스 랜딩, 로그인 `/`은 프로젝트 콘솔이며 재방문 콘솔은 목록을 먼저 보여준다.**

**디자인 이슈 진행 상태 (2026-08-01):**

- **#85 공개판 메인 화면** — **`feat/public-home-85`에서 구현 완료, PR #88 검증/리뷰 대기.** 새 경로를 만들지 않고 `/`을 세션에 따라 분기한다. 비로그인에는 서비스 소개·사용 흐름·`/guide`·`/sample` 진입점을, 로그인에는 기존 콘솔을 보여준다. 프로젝트가 있으면 목록 우선·생성 폼 접힘, 없으면 생성 폼 우선·펼침이다
...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - **#87** 전체 UI 스타일 — 미니멀리즘 적합성 검토·공통 시각 규칙·상태·접근성 일관성 적용 (사용자 등록, **착수 대기**). #85·#86과 범위 경계는 이슈 본문 참조
  - **#86** 산출물 UI — 스크롤 과다·빈 공간·좌측 네비와 흐름도 중복 (사용자 등록, **착수 대기**). 상세는 맨 위 "다음 세션 시작점"
  - **#85** 공개판 메인 화면 디자인 — **구현 완료·PR #88 검증/리뷰 대기** (`feat/public-home-85`)
...
- [ ] T85-6 PR·Preview·main 병합 — **PR #88 생성(Ready for review)**, Preview·리뷰·main 병합 대기. main 병합은 건별 명시적 동의 필요
