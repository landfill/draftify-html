...
**▶ 다음 세션 시작점 (2026-07-31 갱신) — #43 구현·로컬 검증 완료. 작업 브랜치의 Preview 확인과 PR이 남음.**

> **핸드오프 요약 (이 줄부터 5줄만 읽어도 이어받을 수 있게)**
> 1. `main`이 배포 브랜치다. **머지 = 프로덕션 반영.** 작업은 `main`에서 딴 토픽 브랜치 → PR → 머지.
> 2. **#43 B안(ZIP 다운로드) 구현·로컬 검증 완료** — 브랜치 `feat/extension-download-43`, 아직 미커밋·미푸시. Vercel **Preview 환경변수 3종은 활성화됐다고 사용자 확인(2026-07-31)**. 다음은 요청 시 커밋·푸시 → Preview 실다운로드 확인 → PR이다. #45는 **2번(Storage 직접 업로드)이 남아 이슈가 열려 있다**.
...
- **다음 할 일 — 우선순위 재정렬 (2026-07-30, 사용자 결정).** 이전 기록은 `#43 → #45`였다. **화면에 직접 보이는 것을 먼저** 처리한다는 사용자 판단으로 #50·#51을 선두에 두고, 그 뒤를 실피해·비용 순으로 잇는다:
  ...
  5. **#43 확장 배포 경로 — B안 구현·로컬 검증 완료.** 사이트 ZIP 다운로드·개발자 모드 설치 가이드·버전 규칙을 `feat/extension-download-43`에 구현했다. A(웹스토어)는 심사·비용·개인정보 페이지가 딸려오므로 별건. 남은 것은 커밋·푸시 후 Preview 실다운로드와 PR.
...
- **열린 이슈 (8건)** — 처리 순서는 위 "다음 할 일":
  ...
  - **#43** 공개판 확장 배포 경로 — **B안 구현·로컬 검증 완료, `feat/extension-download-43` 미커밋.** Preview 실다운로드·PR·병합 전이라 이슈는 열려 있음
  - **#46** 매직링크 크로스디바이스 — **⛔ 대기(SMTP 제약, 사용자 결정 필요)**
  - **#48** 목업 오리진 격리 — **조건 대기**(외부 공유를 열 때가 조건. 미리 하면 CORS 전환 비용을 쓸 곳 없이 지불)
  - **#12 · #13** `[실수요 대기]` 유지
  - 종결: **#47(PR #58)**, #50·#51(PR #53), #34·#36·#42(이전 세션)
...
- [ ] T43-6 배포 검증 — 커밋·푸시 후 Vercel Preview에서 실 ZIP 다운로드와 실 Supabase 경로 D E2E 확인 → PR·병합(사용자 동의 필요)
...
- 다음 할 일: 사용자가 커밋·푸시를 요청하면 Preview에서 실제 ZIP 다운로드·압축해제·unpacked 로드와 실 Supabase 경로 D E2E를 확인하고 PR을 연다. `main` 병합은 별도 동의를 받는다.
- 막힌 지점: 코드 블로커 없음. 검증 공백은 이 Windows 클론의 `apps/web/.env.local` 부재로 인한 실 Supabase E2E 6본이며, Preview 환경변수 자체는 활성화 확인됨. 기존 빌드 경고(`single-file-core` IIFE의 `import.meta`, 홈 디렉터리 lockfile로 인한 Next workspace root 추론)는 이번 변경과 무관하다.
