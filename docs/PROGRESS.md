...
**▶ 다음 세션 시작점 (2026-08-06 재갱신) — **#105 종결 (PR #108 병합 `8294bc6`).** `storage_object_owned_by_user`의 **anon EXECUTE를 회수**해 advisor 0028을 없앴다. **#57에서 만든 AGENTS.md 6절 "스키마를 바꿀 때" 절차를 처음 적용한 건이다** — 정본 먼저 → 마이그레이션 1본 → 순서 판단 → 동의 후 적용 → advisor 확인. **재사용할 사실: `revoke ... from public`은 PUBLIC 의사역할만 지운다.** Supabase default privileges가 public 스키마 신규 함수에 anon·authenticated·service_role EXECUTE를 **명시적으로** 주므로, 역할을 하나씩 명시해 회수해야 한다. **남은 advisor 2건은 의도적 수용이고 `schema.sql`에 그렇게 적혀 있다** — 0029(authenticated)는 정책이 이 함수를 호출하므로 회수하면 업로드가 막힌다, 0008은 설계대로다. **열린 이슈가 전부 조건·시점 대기라 지금 착수 가능한 것이 없다** — 다음 작업은 사용자 판단이 필요하다. 아래는 그 앞의 기록이다: **#57 종결 (PR #107 병합 `dee86bb`).** **공개판 스키마의 정본은 이제 `apps/web/supabase/schema.sql` 한 파일이다** — "지금 프로덕션 스키마가 무엇인가"의 답. `migrations/`는 적용 수단으로 남고, 변경 절차는 **AGENTS.md 6절 "스키마를 바꿀 때"** 가 규약이다. **정본은 읽기용이고 실행 검증을 하지 않는다** — 환경이 하나뿐이고 **개발용 분리 계획도 없다**(사용자 확정). 세워야 할 일이 생기면 `migrations/`를 쓴다. **파급 1건: #97 A안(테스트 전용 Supabase)이 폐기됐다** — 전제가 사라져 **C안(수동 `test:e2e:web`)이 임시가 아니라 확정**이다. 다시 검토하지 말 것. **리뷰가 내 실수 4건을 전부 잡았다**(기각 0건) — 특히 **PR 본문에 "apps/web을 안 건드렸다"고 적었는데 거짓이었고**(`apps/web/supabase/` 아래를 고쳤다), **파일명을 바꾸고 README 런북을 안 고쳐 거짓 문서를 남길 뻔했다.** **다음 세션은 아래 "열린 이슈"에서 고른다** — 전부 조건·시점 대기라 사용자 판단이 필요하다. 아래는 그 앞의 기록이다: **#103 종결 (PR #104 병합 `27fbbd7`).** 내보내기 전 미저장 편집 선저장 = 킥오프 11절 **22차**. **리뷰가 내 구현에서 결함 2건을 더 찾았고 둘 다 실질이었다** — ① 진행 중 PUT 미직렬화(Codex P1·CodeRabbit Major가 독립 지적) ② 부팅 큐 재전송 실패 시 `lastSyncedRef`가 로컬 기준으로 남아 선저장이 통째로 건너뛰어짐(Codex P2). **둘 다 원래 증상과 같은 "조용한 유실"을 남기는 결함이라 선저장만으로는 문제가 안 풀렸다.** 교훈: **"저장이 실제로 완료됐는가"를 판정하는 자리가 여러 곳이면 한 곳만 고쳐서는 안 된다.** 검증은 `vitest` **417 passed** · 사내판 E2E 4본 · 공개판 E2E 8 passed/skipped 0. **다음 세션은 아래 "열린 이슈"에서 고른다** — 조건 없이 열린 것이 없으므로 `#57`(마이그레이션 통합, 시점 대기) 또는 `#46`(매직링크, 사용자 결정 대기) 중 사용자 판단이 필요하다. 아래는 그 앞의 기록이다: **#99 종결 (PR #102 병합 `bdf0783`).** 검증은 `vitest` 411 passed · 사내판 E2E 4본 · **공개판 E2E 8 passed/skipped 0**. 봇 3종 리뷰를 받았고 **미처리 지적은 0건**이다 — CodeRabbit의 날짜 지적 2건은 UTC 기준 오판이라 기각했고 **봇이 "리뷰 의견을 철회합니다"로 스스로 물렸다**(같은 부류 여섯 번째). Codex의 P2 1건은 **이 PR이 만든 문제가 아니라 편집 필드 전체에 걸린 기존 조건**이라 [#103](https://github.com/landfill/draftify-html/issues/103)으로 분리했다. **다음 세션은 아래 "열린 이슈"에서 고른다** — 우선순위는 `#103`(저장·export 경합. 실피해가 조용해서 나쁘다) → `#57`(마이그레이션 통합). 아래는 #99 착수 당시 기록이다. **이슈 본문의 진단이 실제 증상과 달랐다 — 본문만 읽고 시작하지 말 것.** 실제 증상은 "캡처는 모바일 모양인데 `captureWidth`가 창 폭이라 뷰어가 그 폭으로 그려 좌우가 빈다" = **렌더 폭 문제**이고, 본문이 전제한 iframe 셸·확장 debugger는 **불필요하다.** 정리는 [#99 코멘트](https://github.com/landfill/draftify-html/issues/99#issuecomment-5197244021)·[후속](https://github.com/landfill/draftify-html/issues/99#issuecomment-5197738166)과 맨 아래 세션 로그에 있다. 아래는 그 앞의 기록이다: **#90 종결.** 스크린샷 4장을 2x로 재촬영해 `main`에 반영했고(`79ae30e`·`e5d111c`), 이슈 본문의 옛 진단도 정정한 뒤 닫았다. 그 앞의 #97은 C안(수동 실행 규약화)으로 종결됐고(merge `7a0196b`), T87-14로 **네 표면이 모두 `THEME_TOKENS`를 쓴다**(merge `54ece29`). **다음 세션은 아래 "열린 이슈"에서 골라 시작하면 된다** — 우선순위는 `#99`(대상 기기) → `#57`(마이그레이션 통합. **끝나면 #97 A안 재검토 조건이 된다**). UI를 건드리기 전에 `docs/ui-standard.md` **3.0절(기각 이력)** 과 **6.5절(포커스 링 함정)** 을 반드시 읽는다. **스크린샷을 다시 찍을 일이 생기면 아래 세션 로그의 `deviceScaleFactor: 2` 경고를 먼저 읽는다.**

> **⚠️ 산출물 시각 언어는 킥오프 20차가 정본이다 (T87-14, PR #101).** `export.ts`가 `THEME_TOKENS`를
> 빌드타임 import해 `:root`에 라이트 토큰을 찍는다 — **색을 손으로 적지 않는다.** 남은 리터럴은
> 경고색(`#fbbc04`·`#b06000`, 팔레트에 warn 토큰이 없다)과 의도적 흰색뿐이다. 라이트 고정이고
> 다크는 선언하지 않는다(목업 캡처가 캡처 시점 색으로 굳어 있어 셸만 뒤집으면 문서 안에서 두
> 테마가 부딪힌다). 상세는 `ui-standard.md` **6.8절**.
>
> **마커의 포커스 링만 `box-shadow`로 전달한다.** `.ms-marker.is-active`가 `outline` 슬롯을 앰버
> 활성 표시에 쓰고 있어, 포커스도 outline이면 둘 중 하나가 사라진다. **`.is-active`를
> `.ms-marker:focus-visible`보다 뒤에 둬야 한다** — 특정도가 같아(0,2,0) 순서가 승부를 가른다.
>
> **⚠️ 산출물 색은 `export.ts` 한 곳에만 있지 않다.** 흐름도 화살촉은
> `packages/viewer/src/main.ts`가 인라인 `fill` 속성으로 칠한다 — 초안이 그걸 놓쳐 간선만
> 슬레이트로 가고 화살촉은 Google 그레이로 남았다(PR #101 Codex 지적). 뷰어는 산출물에 단일
> 모듈로 인라인돼 `@mockspec/shared`를 import할 수 없으므로 **토큰을 참조할 유일한 경로가 CSS**다.
> 다음에 색을 다루면 `grep -nE '#[0-9a-fA-F]{3,8}' packages/viewer/src/main.ts`도 함께 본다.

> **⚠️ 산출물 레이아웃은 킥오프 19차가 정본이다 (#86).** 좌측 화면목록 제거 · 화면영역 하단
> 전/후 컨트롤 · 흐름도 기본 접힘(점프 전용) · 캡처 fit 축소. **되돌리기 전에 19차의 "이유"
> 칸을 읽는다** — 특히 ③(기본 접힘)은 ②(전/후 컨트롤)가 **선행 조건**이다. 전이가 없으면
> 흐름도가 렌더되지 않으므로(`edges.length === 0`), 흐름도를 유일한 이동 수단으로 두면
> 전이 미입력 프로젝트는 2번째 화면부터 도달 불가가 된다.
>
> **fit 배율은 안쪽 `.ms-stage-scale`에 건다. 테두리를 가진 `.ms-stage-wrap`을 배율 안에
> 넣지 않는다** — 전역 `box-sizing: border-box` 탓에 콘텐츠 상자가 테두리만큼 작아져
> 축소본이 잘린다(실측 1.58px). 두 판은 `buildExportHtml`·`VIEWER_CSS`·`viewer/dist/main.js`를
> **공유**하므로 한쪽만 고치는 상황이 아니다.

> **⚠️ 공개판 E2E(`test:e2e:web`)는 CI에서 돌지 않는다 — 손으로 돌리는 것이 규약이다 (#97 → C안 확정, PR #100).**
> `.github/workflows/ci.yml:45`는 `npm run test:e2e`(사내판)만 실행한다. **공개판이나 두 배포가
> 공유하는 코드(`packages/server/src/routes/export.ts`·`shared`·`viewer`)를 건드리면 `npm run test:e2e:web`을
> 로컬에서 직접 돌린다** — 안 돌리면 아무도 안 돌린 것이다. #91이 정확히 그 공백으로 PR #88부터
> 하루 넘게 살아남았다. **`skipped`가 하나라도 있으면 검증한 것이 아니다** — 게이트는 파일 존재가
> 아니라 값이다(`open-service-dod.spec.ts`의 `hasEnv`: URL·publishable 키 + `sb_secret_` 접두).
> **대상 패키지는 외우지 말고 뽑는다** — `apps/web`이 끌어오는 것이 트리거이고(오늘 기준
> `extension`·`sdk`·`server`·`shared`·`viewer`), 이들 대부분이 **빌드 시점에 공개판 번들 안으로
> 들어간다**(`sdk-bundle.ts`·`viewer-script.ts`·`build-export.ts`). 규약 본문과 도출 명령은
> **AGENTS.md 6절 "병합 전 검증"**(PR #100 병합 후 유효).

> **⚠️ 봇 리뷰 3종을 다 받았는지 확인한다 (#96에서 얻은 것).** CodeRabbit·PR-Agent는 PR 오픈에
> 자동으로 붙지만 **Codex는 붙지 않는 경우가 있다** — PR #96은 8분간 무반응이었고 PR #95·#89에는
> Codex 코멘트가 아예 없다. 봇 안내문은 "PR 오픈 시 자동 실행"이라 적혀 있지만 **믿지 말고
> `@codex review`를 손으로 건다**(응답까지 약 2분). 이 저장소에서 Codex는 다른 둘이 놓친 것을
> 잡은 이력이 있다 — #91의 원인이 된 767행 처리 자체가 **PR #88의 Codex P2 지적**에서 나왔다.

> **⚠️ 가로 오버플로 0을 반응형 통과로 읽지 않는다 (#93에서 얻은 것).** `flex: 1 1 0` +
> `min-width: 0`인 요소는 넘치는 대신 **0px까지 눌려 사라지므로** 그 지표에 잡히지 않는다.
> 공개판이 "390px에서 0이라 문제없다"고 기록돼 있었지만 실측하니 이름 칸이 9px였다.
> 좁은 폭 검증에는 **주요 정보 요소의 실제 렌더 폭을 함께 찍는다** (`ui-standard.md` 7절).

> **⚠️ 병합 직전에 온 리뷰를 놓치지 말 것.** PR #92는 Codex 3차 지적이 `2026-08-02 14:50:28`에 도착했고 병합이 `14:50:51`이었다 — 23초 차이로 그 PR에 담지 못해 PR #94로 따로 처리했다. `gh pr merge`를 실행하기 전에 `--json reviews`와 `gh api .../pulls/{n}/comments`를 **마지막으로 한 번 더** 확인한다.
>
> **⚠️ `docs/PROGRESS.md`만 바뀐 커밋에는 `@codex review`를 걸지 않는다** (사용자 지시). AGENTS.md 6절이 이 파일을 "리뷰 대상이 아니라 다음 세션이 읽을 상태 기록"으로 규정한 이유가 그것이다. PR #94에서 이를 어겼더니 **봇 지적 4건이 전부 이 파일의 산문**이었고, 정작 실질 변경인 CSS 한 줄은 아무도 다시 보지 않았다. **코드가 딸린 작업이면 PROGRESS는 병합 후 `main`에 직접 커밋하는 쪽이 낫다** — 이 항목이 그 방식으로 기록됐다.

**디자인 이슈 진행 상태 (2026-08-02):**

- **#87 전체 UI 스타일 — 완료·종결 (PR #92 병합 `aa839cb`, 2026-08-02).** T87-1~9는 PR #89(`56b7735`), T87-10~13은 PR #92다. **T87-14(산출물)는 #86 종속이라 이 이슈에서 하지 않기로 문서 0절에 정해져 있다.** ⚠️ A/B 검증에서 "본문 글자를 키우는" 방향이 세 번 기각됐다 — 콘솔·가이드·FAQ·랜딩 프리뷰는 현재 유지다. 채택된 것은 구조적 결함을 고친 경우뿐. 상세는 `docs/ui-standard.md` 3.0절
- **#90 README 스크린샷 노후화** — **완료·종결 (`79ae30e`·`e5d111c` main 직접 반영, 2026-08-06).** 등록 당시(2026-08-02) 진단이 두 곳 어긋나 있었다: ① **1번째는 노후가 아니었다** — 근거로 든 #43(확장 ZIP 배포)·#85(생성 폼 `<details>` 접힘)은 **둘 다 공개판 전용 변경**이고(`apps/web/lib/extension-release.ts`·`components/console-home.tsx`), 사내판 콘솔은 지금도 `설치(packages/extension/dist)` 안내에 폼이 펼쳐진 채다 ② **"2·5번째는 유효"가 뒤집혔다** — 4번째는 T87-13 이전에 이미 `d5f2d44`(동결→**캡처** 용어)·#38(페이지 헤더 밴드)·#31(SPA 배너)로 낡았고, 5번째는 #86이 **3컬럼→2컬럼**으로 갈아엎었다. **실제로 무변경인 것은 2번째(확장 팝업) 하나뿐**(`popup.html` diff 0건, `binding.ts` 주석 1줄). 사용자 방침대로 **2번째만 남기고 1·3·4·5를 새로 찍었다**
- **#93 콘솔 목록 좁은 화면** — **완료·종결 (PR #95 병합 `0eeea4b`, 2026-08-03).** 두 판 모두 720px 이하에서 목록 행을 세로로 접는다. **등록 당시 진단이 두 곳 어긋나 있었다**: ① 넘침(+67px)은 증상의 절반이고, 진짜 피해는 `.c-project-info`가 0px로 눌려 **프로젝트 이름이 사라지는 것**이었다(오버플로 0이던 480px도 info는 0px) ② **"공개판은 0이라 괜찮다"는 틀렸다** — 실측하니 390px에서 info 9px, 320px에서 0px. 넘치지 않은 이유는 버튼이 2~3개여서지(사내판 5개) 결함이 없어서가 아니었다. 상세는 `ui-standard.md` 6.7절
- **#85 공개판 메인 화면** — **완료·종결 (PR #88 병합 `ea3d377`, 2026-08-01).** 새 경로를 만들지 않고 `/`을 세션에 따라 분기한다. 비로그인에는 서비스 소개·사용 흐름·`/guide`·`/sample` 진입점을, 로그인에는 기존 콘솔을 보여준다. 프로젝트가 있으면 목록 우선·생성 폼 접힘, 없으면 생성 폼 우선·펼침이다
...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - ~~**#105**~~ `storage_object_owned_by_user`를 anon이 실행할 수 있다 — **완료·종결 (PR #108 병합 `8294bc6`, 2026-08-06).** `revoke execute ... from anon` 1줄. 적용 후 ACL은 `postgres | authenticated | service_role`이고 **advisor 0028은 소멸**했다. **authenticated·service_role은 의도적으로 남겼다** — 전자는 정책이 이 함수를 호출해 회수하면 업로드가 전부 막히고(0029 수용), 후자는 RLS 우회라 불필요하지만 회수해도 이득이 없다. `apply_migration`으로 적용해 원격 이력에 `20260806143202`로 남겼고 **로컬 파일도 같은 버전으로** 만들어 #57의 정합(로컬=원격, 8본=8행)을 유지했다. **실질 회귀 검증은 공개판 E2E 1번**(가입→업로드→편집→export→뷰어) — 인증 사용자 업로드가 통과하면 정책이 함수를 정상 호출한다는 뜻이다. 아래는 등록 당시 기록이다: **신규 등록 (2026-08-06, #57 조사에서 분리).** 의도는 authenticated 전용인데 실제 ACL에 anon·service_role이 있다. **영향은 낮다**(anon은 `auth.uid()`가 null이라 항상 false 반환) 단 advisor 0028이 WARN으로 남는다. 조치는 `revoke execute ... from anon;` 한 줄이지만 **프로덕션 권한 변경이라 사용자 동의가 선행**한다. 적용하면 `schema.sql`의 "의도≠실제" 주석도 함께 지운다
  - ~~**#103**~~ 편집 직후 내보내면 마지막 편집이 산출물에 빠진다 — **완료·종결 (PR #104 병합 `27fbbd7`, 2026-08-06).** 킥오프 11절 **22차**로 확정: `runExport`가 export 전에 미저장 편집을 PUT하고, 실패 시 확인 다이얼로그로 사람에게 묻는다(차단 아님). **원인이 3층이었고 리뷰가 2·3층을 찾아줬다** — ① 저장은 500ms 디바운스인데 export는 서버의 마지막 저장본을 읽는다(선저장으로 해결) ② **디바운스 PUT은 추적되지 않고 effect cleanup은 타이머만 취소한다** — 이미 날아간 요청은 못 막아 겹친 PUT이 최신 편집을 덮는다(직렬 체인 `saveChainRef`로 해결, Codex P1·CodeRabbit Major) ③ **부팅 큐 재전송이 실패하면 `lastSyncedRef`가 로컬 큐 내용으로 잡힌 채 남아** 서명 비교가 "동기화됨"으로 오판하고 선저장을 건너뛴다(큐 확인으로 해결, Codex P2). **재사용할 교훈 3가지**: ⓐ `lastSyncedRef`는 "서버가 가진 것"을 뜻하는데 **부팅 경로가 그 불변식을 깬다** — 다음에 이 값을 읽는 코드를 추가하면 큐 잔존 여부를 함께 봐야 한다 ⓑ **모든 PUT은 `enqueueSave` 체인을 통과해야 한다**(디바운스·선저장·오프라인 재전송·부팅 재전송 4경로). 새 저장 경로를 추가하면 체인에 태운다 ⓒ **버튼을 `saveStatus`로 막는 방식은 기각됐다** — 오프라인 큐 상태에서 이미 저장된 내용까지 못 내보내게 되어 큐 장치의 목적과 충돌한다. **검증 방식도 남긴다**: 회귀 테스트마다 **해당 수정만 되돌려 실제로 실패하는지 확인**했다(공허한 테스트 방지). CodeRabbit의 "`N차` 순번 표기를 쓰지 말라"는 기각했고 봇이 철회했다 — 저장소 **196곳**(스펙 48·PROGRESS 36·기타 문서 44·코드 주석 68)이 이 번호로 상호 참조한다. **미결 1건(사용자 조치)**: CodeRabbit에 "AGENTS.md가 개정 번호를 요구한다"는 **부정확한 학습이 저장됐고 봇이 스스로 못 지운다** — CodeRabbit UI의 Learnings 페이지에서 삭제해야 다음 PR에서 그 전제로 리뷰가 나오지 않는다
  - ~~**#99**~~ 산출물에 대상 기기 개념이 없다 — **완료·종결 (PR #102 병합 `bdf0783`, 2026-08-06).** 킥오프 11절 21차로 `Scene.targetDevice`(mobile 390px / desktop·미지정은 현행이라 **기존 스펙의 동작 변화 0**)를 확정하고 `shared`·`sdk`·`viewer`에 구현했다. **뷰어의 390 상수는 의도적 복제**(산출물은 단일 모듈 인라인이라 `@mockspec/shared` 런타임 import 불가 — 18차와 같은 이유)이고 드리프트는 `sceneDisplay.parity.test.ts`가 잡는다. **`export.ts`는 건드리지 않았다** — iframe 폭은 JS가 인라인으로 지정한다. 아래는 착수 당시 조사 기록이다. **실제 증상은 본문 진단과 다르다** — 캡처 콘텐츠는 이미 모바일 모양이고, `captureWidth`가 창 폭이라 **뷰어 렌더 폭**이 넓어 좌우가 비는 것이다(사용자 확인). 따라서 iframe 편집 셸·확장 debugger는 **불필요**하고, 파일 4개(`shared`·`sdk`·`viewer`·`export`)로 끝난다. 남은 결정: 모바일 폭 값(390 권장) · 적용 단위(`Scene` 필드 + 직전 값 프리필 권장). [비용 정리](https://github.com/landfill/draftify-html/issues/99#issuecomment-5197244021) · [증상 확인·범위 축소](https://github.com/landfill/draftify-html/issues/99#issuecomment-5197738166). 조사에서 확인된 것(다른 증상을 다룰 때 재사용): ① A·B도 iframe이 아니라 새 탭 최상위라 **어느 경로도 캡처 폭 강제가 안 된다** ② 캡처가 브레이크포인트를 지워(`removeUnusedStyles`·`removeHiddenElements`) **반응형 화면은 사후에 폭만 줄이면 깨진 데스크톱이 된다**(실측: `column` 전환 0건). 아래는 등록 당시 기록이다: **신규 등록 (2026-08-05, #86 논의 중 분리).** `captureWidth`가 `documentElement.clientWidth`(기획자 브라우저 창 폭)라, 모바일 화면을 기획해도 데스크톱에서 캡처하면 PC 레이아웃이 굳는다. **#86의 fit 축소는 좌우 스크롤만 없앨 뿐 증상의 절반만 덮는다.** 경로 D(확장)는 사용자의 실제 페이지 위에서 돌아 창 폭을 강제할 수 없어 방향이 갈린다. `Scene` 필드 추가는 AGENTS.md 4절 절차 필요
  - ~~**#90**~~ README 경로 D 스크린샷 노후화 — **완료·종결 (`79ae30e`·`e5d111c`, 2026-08-06).** 무변경은 2번째(확장 팝업) 하나뿐이었고 1·3·4·5를 2x로 새로 찍었다. 등록 당시 진단의 어긋남은 위 "디자인 이슈 진행 상태" 참조
  - ~~**#97**~~ 공개판 E2E가 CI에서 한 번도 실행되지 않는다 — **완료·종결 (PR #100 병합 `7a0196b`, 2026-08-06).** C안(수동 실행 규약화)으로 결정(사용자)해 AGENTS.md 6절 "병합 전 검증"이 됐다. A안(테스트 전용 Supabase)은 **#57이 끝난 뒤 재검토**한다 — 마이그레이션 이중 적용 비용이 그때 내려간다. 아래는 등록 당시 기록이다: **신규 등록 (2026-08-05, #91 규명 중 발견).** `main` 병합 = 프로덕션인 쪽에 자동 회귀 그물이 없다. **한 줄 추가로 끝나지 않는다**: 키가 없으면 8본이 조용히 전부 스킵되고, 키를 넣으면 Preview·Production이 공유하는 **프로덕션 DB에 푸시마다 테스트 사용자·프로젝트가 쌓인다.** 선택지 A(테스트 전용 Supabase·권고) / B(프로덕션 DB + 정리 강제) / C(수동 실행 규약) 중 **택일이 제품 결정.** A는 #57과 맞물린다(먼저 하면 비용이 내려간다)
  - ~~**#57**~~ 마이그레이션을 최종 배포 스키마 기준으로 통합 관리 — **완료·종결 (PR #107 병합 `dee86bb`, 2026-08-06).** 정본 = `apps/web/supabase/schema.sql` 1본, `migrations/`는 적용 수단. **선언적 스키마(`db diff`)를 기각한 이유**: 이 스키마의 보안 핵심 3개가 그 사각지대다 — 버킷 행 삽입(데이터 seed) · `storage.objects` 정책(Supabase 관리 스키마) · 함수 grant/revoke(권한 diff 부분 지원). squash도 기각(압축 직후에만 참). **조사에서 드러난 것 2건**: ⓐ 로컬 7본은 실 스키마를 정확히 재현하며 **어긋난 것은 스키마가 아니라 이력 테이블뿐이었다** ⓑ `storage_object_owned_by_user`를 **anon이 실행 가능**하다(의도는 authenticated 전용) — `revoke ... from public`은 PUBLIC 의사역할만 지우고 Supabase default privileges가 준 명시적 anon grant는 안 지운다. advisor 0028 WARN, **#105로 분리**. **이력 정리 실행**: 원격 `supabase_migrations`에 누락된 `20260724150000_fix_storage_object_rls` **INSERT 1행**(DELETE·UPDATE 없음), 로컬 5본은 원격 실제 버전으로 개명 → 로컬 7본 = 원격 7행 파일명까지 일치. **개명 대응표는 `apps/web/README.md`에 있다** — PROGRESS의 옛 기록은 당시 파일명을 그대로 쓰므로 그 표로 번역한다. **완료 조건 3번(새 환경 재현 실증)은 조건에서 뺐다**(#106 `not planned` 종결) — 새 DB를 만드는 경로가 저장소에 없다(CI 워크플로 supabase 참조 0건, `config.toml`은 "로컬 DB 미사용"). `config.toml`의 `major_version`도 15 → **17**로 실제와 맞췄다
  ...
  - **#12 · #13** `[실수요 대기]` 유지
  - 종결: **#105(PR #108)**, **#57(PR #107)**, **#103(PR #104)**, **#99(PR #102)**, **#90(main 직접 `79ae30e`·`e5d111c`)**, **#97(PR #100)**, **#87 T87-14(PR #101)**, **#86(PR #98)**, **#91(PR #96)**, **#93(PR #95)**, **#87(PR #92·#94)**, **#85(PR #88)**, **#81(PR #84)**, **#82(PR #83)**, **#71(PR #80)**, #64·#70(PR #79), #77(PR #78), #65(PR #76), #68(PR #75), #63(PR #72), #43(PR #67), #47(PR #58), #50·#51(PR #53), #34·#36·#42(이전 세션)
...
## 공개판 메인 화면 WBS 체크리스트 (이슈 #85, 2026-08-01 착수 — **완료·main 병합**)
...
- [x] T85-6 PR·Preview·main 병합 — **PR #88 병합 완료 (merge `ea3d377`, 2026-08-01), 이슈 #85 종결.**
...
## 전체 UI 스타일 WBS 체크리스트 (이슈 #87 — **완료·종결 2026-08-02.** PR #89 `56b7735` + PR #92 `aa839cb`)

> 상세는 [ui-standard.md](./ui-standard.md). **3.0절의 기각 이력을 먼저 읽는다.**
> 산출물 코드는 범위 밖(#86) — 문서 0절 참조.

- [x] T87-1 표면 인벤토리 — 네 표면의 팔레트·폰트·토큰화·다크·반응형·스케일 측정 (문서 1절)
- [x] T87-2 결함 확인 — F-01~F-05, 대비는 WCAG 계산값으로 검증 (문서 2절)
- [x] T87-3 공통 시각 기준 제안 (문서 4절)
- [x] T87-4 방향 확정 — **A/B 5회차로 결정.** 본문 크기를 키우는 방향은 채택되지 않는다
- [x] T87-5 문서 — `docs/ui-standard.md` 신설 + `docs/README.md`·`AGENTS.md` 1절 등록
- [x] T87-6 로그인 화면 — `<h1>` 13px → **22px**, 버튼 위계 분리, 인라인 style 4곳 제거, 서비스 설명 1줄
- [x] T87-7 랜딩 히어로 중앙 정렬 — 미리보기 `max-width: 860px`. CSS만, 마크업 무변경
- [x] T87-8 PR 생성·리뷰 반영 — **PR #89 하나로 통합** (사용자 요청). 봇 리뷰 11건 처리(9건 반영·2건 기각), CI 6체크 green
- [x] T87-9 main 병합 — **완료 (merge `56b7735`, 2026-08-02).** CI 6체크 green, 원격 브랜치 삭제·로컬 정리·main 동기화까지 완료
- [x] T87-10 F-01 `--c-faint` 대비 수정 — **Q-04를 "토큰 통합"으로 결정(사용자)해 토큰을 삭제하고 사용처 4곳을 `--c-muted`로 옮겼다.** `globals.css`·`shell.ts`·`console.ts`·`pages.ts` 동시. 위계 보완이 필요했던 곳은 `.g-path dt` 하나뿐(`font-weight: 500`) — 나머지 셋은 크기·굵기·서체·위치가 이미 구별을 만들고 있었다. 상세·대비 계산표는 `ui-standard.md` 6.3절
- [x] T87-11 F-02 토큰 단일 원천 — `packages/shared/src/theme.ts`의 `THEME_TOKENS`가 정본. `shell.ts`는 `themeTokenDeclarations()`로 찍어 **복제가 사라졌고**, `globals.css`는 파일로 두고 `theme.parity.test.ts`가 묶는다. **SDK는 하지 않았다** — `:host`가 필요하고 팔레트도 달라 T87-13에서 한 번에 간다. 상세는 `ui-standard.md` 6.4절
- [x] T87-12 F-03 포커스 통일 — **공개판까지 넣었다**(실측해 보니 공개판 버튼·링크도 브라우저 기본 링이라, 사내판만 고치면 두 배포가 새로 갈린다). 링 형태 **4종 → 2종**. `transition: all`이 포커스 링을 0.15초 늦추던 것도 함께 고쳤다. 상세는 `ui-standard.md` 6.5절
- [x] T87-13 SDK 패널 — **Q-03을 "라이트 고정"으로 결정(사용자).** `:host`에 `THEME_TOKENS.light` 선언, 강조색 `#2f6feb`→`--c-accent`, 임의 회색 11종→slate 토큰, 반경 **7종→3종**(4·8·999px). 크기·간격은 그대로. 선택 마커의 기존 대비 미달(4.30:1)도 함께 고쳤다. 상세는 `ui-standard.md` 6.6절
- [x] T87-14 산출물 — **#87을 닫을 때는 범위 밖이었으나(착수 조건 = #86 완료 + AGENTS.md 4절 절차), #86이 19차로 끝나 조건이 충족돼 2026-08-06에 따로 처리했다.** 킥오프 **20차** → 문서 동기화 → 구현. 강조색·중립 회색·헤더 그라디언트·반경·포커스 링을 한 번에 정리해 **네 표면이 모두 `THEME_TOKENS`를 쓴다.** 상세는 `ui-standard.md` 6.8절. **PR #101 병합 완료 (`54ece29`, 2026-08-06)**

## 세션 로그 (최신이 위)

### 2026-08-06 — #99 착수: 세 방향 비용 정리 — 이슈 본문 전제 두 개가 실측으로 뒤집혔다

완료:
- **#99 조사·비용 정리를 이슈 코멘트로 등록했다** ([#99 comment](https://github.com/landfill/draftify-html/issues/99#issuecomment-5197244021)). 코드 0줄. **본문의 전제 두 개가 틀린 것을 확인해 안 배치 자체를 바꿨다:**
  - **정정 ① "경로 A·B는 서비스가 iframe으로 띄운다 → 폭 강제 가능"이 사실이 아니다.** 목업은 **최상위 문서로 새 탭**에 열린다 — 사내판 `console.ts:455`(`openLink.target="_blank"`), 공개판 `console-home.tsx:13`(`/m/{id}/`). SDK는 그 최상위에서 돌며 `App.tsx:603`이 `documentElement.clientWidth`(=브라우저 창 폭)를 읽는다. **지금은 A·B에서도 폭 강제가 불가능하고**, 가능하게 하려면 iframe 편집 셸을 신설해야 한다 — 본문은 이를 "가능"으로 적어 2안을 크게 과소평가했다
  - **정정 ② "캡처해 두고 뷰어에서 폭만 줄이면 된다"는 성립하지 않는다** (가장 값싼 길이라 먼저 실측했다). **정적 근거**: `prj_bcj1u000qw`(1470px 캡처)의 목업 원본 CSS 브레이크포인트는 `min-width` 40·48·64·80·96rem **5종인데 스냅샷에는 40·48rem 2종만 남았다** — `freeze.ts`의 `removeUnusedStyles: true`·`removeHiddenElements: true`가 캡처 시점 기준으로 규칙과 요소를 지운다. **동적 근거**: SCR-003 스냅샷을 Playwright로 1470/390px 렌더 → **가로 flex/grid 컨테이너 9개가 390px에서도 그대로 9개**(`column` 전환 0건), 보이는 요소 108→105. 하단 네비가 눌려 라벨이 잘리고 헤더 `DIRECTOR/Guest`가 사라졌다. **스냅샷은 반응형 문서가 아니라 "그 폭에서의 모습"이다 — 대상 기기는 캡처 시점에 정해져야 한다**
  - **정정 ③ 3안(경고만)은 단독 성립 불가.** 비교하려면 대상 기기를 사람이 입력해야 하고, 캡처 폭·제목에서 추론하는 것은 **설계 제1원칙 1번 위반**이다 → 3안은 1안의 부분집합
- **그래서 "택일 3안"이 아니라 "누적 3단계"로 재구성했다** — 1단계(모델+경고, 중간 비용·파일 5개) → 2단계(A·B 폭 강제, **S1 코어 구조 변경 규모**: SDK를 부모로 올리면 postMessage 브리지 + 앵커·마커오프셋·`focusShield` 전부 영향) → 3단계(D까지: `chrome.windows.update`는 권한 불필요하나 창폭≠뷰포트폭, `chrome.debugger`는 정확하지만 "디버깅 중" 배너 상시 노출 + 웹스토어 심사 부담)
- **권고: 1단계 먼저.** 1단계가 2단계의 선행 조건이고(모델이 있어야 강제할 대상이 생긴다), 2단계는 되돌리기 비싸다

- **⚠️ 그런데 사용자 확인으로 증상이 바뀌었고, 범위가 1단계로 줄었다** ([후속 코멘트](https://github.com/landfill/draftify-html/issues/99#issuecomment-5197738166)). 사용자 진술: **"화면은 모바일 모양으로 잘 나오는데 브라우저 좌우폭이 크게 남은 상태로 기획서가 작성된다."** 즉 **캡처 콘텐츠는 이미 모바일이 맞고**, `captureWidth`가 창 폭(1470)이라 뷰어가 그 폭으로 iframe을 그려 좌우가 비는 것이다 — **캡처 시점 문제가 아니라 렌더 폭 문제.** 따라서 **2단계(iframe 편집 셸)·3단계(확장 debugger·창 리사이즈)는 이 이슈에 불필요하다**(본문이 진단한 "반응형이 PC로 굳는" 증상은 별개이고, 필요해지면 그때 위 비용 산정을 되살린다)
- **피해가 이중이라는 것도 확인했다.** `main.ts:659~665`의 `docWidth = Math.max(..., baseWidth)`가 1470을 채택하면 **#86의 fit 축소(626~640행)가 그 1470을 기준으로 배율을 계산한다** → ① 좌우 970px 여백 ② 그 여백까지 욱여넣느라 **모바일 콘텐츠가 실제보다 더 축소된다.** `targetDevice`로 `baseWidth`를 390으로 잡으면 여백이 사라지고 배율도 1.0으로 돌아온다
- **위 근거 ②(사후 폭 변경 위험)는 이 케이스에 적용되지 않는다** — 그건 반응형 사이트 이야기이고, 이미 모바일로 캡처된 화면은 폭을 줄여도 깨지지 않는다. 안전장치도 현행 코드에 있다(`Math.max`라 안 들어가면 실제 `scrollWidth` 채택 + fit 흡수 → 잘리지 않는다)

- **결정 2건이 나왔고(사용자, 2026-08-06) 스펙·구현까지 마쳤다** — 모바일 폭 **390px**, 적용 단위 **화면별 + 직전 값 프리필**. 브랜치 `feat/target-device-99`, 커밋 `e4b93a0`(스펙)·`52be323`(구현). **`main` 병합은 사용자 동의 대기.**
  - **킥오프 11절 21차 개정** + `technical-spec`·`detailed-spec`·`output-standard` 동기화 (AGENTS.md 4절대로 코드보다 먼저)
  - 구현: `shared`(`TargetDevice`·`MOBILE_RENDER_WIDTH`·`sceneRenderWidth`·`previousSceneTargetDevice`) · `sdk`(패널 기기 선택·`createScene` 프리필·`updateSceneTargetDevice`) · `viewer`(`baseWidth`를 `sceneRenderWidth`로). **export CSS는 건드리지 않았다** — iframe 폭은 JS가 인라인으로 지정하므로 CSS 변경이 필요 없었다
...
- **병합 완료 — 사용자 동의를 받아 `main`에 병합했다 (merge `bdf0783`, 2026-08-06). 이슈 #99 종결.** 병합 후 정리도 끝: 원격·로컬 토픽 브랜치 삭제, `git ls-remote` 기준 원격 브랜치는 `main` 하나뿐, 로컬 `main` 동기화 완료.
  - **CodeRabbit이 지적 2건을 스스로 철회했다.** 기각 답글에 대해 봇이 "제 지적은 UTC 기준 날짜를 잘못 적용했습니다 … **이 리뷰 의견을 철회합니다**", "이 저장소의 진행 기록 날짜는 UTC가 아니라 로컬 KST 기준입니다"라고 답했다. **다음에 같은 지적을 받으면 이 사례를 인용하면 된다** — 기각이 일방 주장으로 남지 않고 봇 쪽에서 확인된 첫 사례다
  - **Codex P2 1건은 [#103](https://github.com/landfill/draftify-html/issues/103)으로 분리했다** ("Persist the device choice before allowing export"). **지적 자체는 사실이지만 이 PR이 만든 것이 아니다** — `runExport`도 저장 디바운스도 이 PR에서 한 줄도 안 건드렸고(`App.tsx` 변경은 import 1줄 + 패널 섹션 하나, hunk 2개) **모든 편집 필드가 동일하게 노출된 기존 조건**이다. 여기서 고치면 별개 범위를 이 PR이 삼킨다. 근거를 [스레드](https://github.com/landfill/draftify-html/pull/102#discussion_r3726222407)에 남겼다

## 세션 로그 — #57 스키마 정본 (2026-08-06)

**PR [#107](https://github.com/landfill/draftify-html/pull/107) 병합 완료 (merge `dee86bb`, 사용자 동의). 이슈 #57 종결.** 병합 후 정리 끝 — 원격·로컬 토픽 브랜치 삭제, 원격 브랜치는 `main` 하나뿐, 로컬 `main` 동기화 완료.

- **커밋 4개**: 정본 도입(`03bd697`) → 절차 복제 제거(`8487c34`) → 재현 실증 조건 제외(`0b96d96`) → 끊긴 참조 정리(`bebcc3e`)
- **프로덕션 DB에 쓴 것은 이력 테이블 INSERT 1행뿐이다.** 스키마·데이터 무변경. 사용자에게 실행 문장을 보이고 동의받은 뒤 실행했다
- **리뷰가 내 실수 4건을 전부 잡았다 — 기각 0건.** 이 세션에서 가장 뼈아픈 둘:
  - **PR 본문에 "`apps/web`을 건드리지 않았으므로 공개판 E2E 대상이 아니다"라고 적었는데 거짓이었다**(Codex P1). `apps/web/supabase/` 아래를 고쳤다. **AGENTS.md 6절이 바로 그 판단("공개판 코드가 아니니 괜찮다")을 틀리는 지점이라 경고하고 있는데 정확히 그 함정에 빠졌다.** 규칙 대상은 경로(`apps/web/**`)지 "공개판 코드인가"라는 판단이 아니다
  - **파일 5본 이름을 바꾸고 그것을 가리키는 README 런북·코드 주석을 안 고쳤다**(Codex P2). 런북이 **이 PR이 해소한 불일치를 미해소로 서술**하고 없는 파일을 가리키게 됐다 — 자기 PR 때문에 문서가 거짓이 된 경우다. **이름을 바꾸면 `grep`으로 참조를 전수 확인한다**
  - 나머지 2건: 절차를 `technical-spec`에 복제(CLAUDE.md "규약은 AGENTS.md 한 곳" 위반) · MD028
- **배경 조회의 시각 필터를 잘못 써서 Codex 지적 2건 중 1건만 보고 "신규 지적 없음"으로 보고할 뻔했다.** 봇 코멘트를 시각으로 거를 때는 **경계를 넉넉히** 잡거나 전체를 나열해 눈으로 본다
- **재사용할 사실**: `db diff`(선언적 스키마)는 **데이터 seed · 미소유 스키마 객체 · 권한**을 못 잡는다. 우리 스키마는 그 셋이 전부 보안 핵심이라 기각 근거가 됐다
- **검증**: `typecheck` 0 · `vitest` **417 passed** · **공개판 E2E 8 passed / skipped 0**(P1 지적 후 실행, 이후 `apps/web`을 또 건드려 재실행)

## 세션 로그 — #103 내보내기 선저장 (2026-08-06)

**PR [#104](https://github.com/landfill/draftify-html/pull/104) 병합 완료 (merge `27fbbd7`, 사용자 동의). 이슈 #103 종결.** 병합 후 정리 끝 — 원격·로컬 토픽 브랜치 삭제, 원격 브랜치는 `main` 하나뿐, 로컬 `main` 동기화 완료.

- **스펙 먼저 고쳤다** (AGENTS.md 4절): 킥오프 §11 **22차 개정** + §6.4, `detailed-spec` §3.9, `technical-spec` §6. technical-spec의 export 엔드포인트 행에 **"서버는 마지막 저장본을 읽는다 — 호출자가 미저장 편집을 먼저 PUT할 책임이 있다"** 를 명시했다. **이 계약이 어디에도 적혀 있지 않았던 것이 결함의 뿌리다.**
- **커밋 5개**: 스펙(`728391c`) → 선저장(`bf42616`) → 직렬화(`e04801b`) → 큐 확인(`7696ae3`) → 테스트 강화(`9aeaf07`)
- **내 첫 구현은 부실했다.** 선저장이라는 방향은 맞았지만 **"저장이 실제로 완료됐는가"를 두 군데서 잘못 판단**하고 있었고, 둘 다 원래 증상과 같은 조용한 유실을 남겼다. 리뷰가 없었으면 그대로 병합됐을 것이다
- **리뷰 지적 4건 중 3건 수용·1건 기각**(기각분은 봇이 철회). 수용 2건은 위 "열린 이슈"의 #103 항목에 원인·해법·재사용 교훈까지 적어 뒀다
- **회귀 테스트마다 해당 수정만 되돌려 실패를 확인했다** — 직렬화를 빼면 직렬화 테스트만, 큐 확인을 빼면 큐 테스트만 실패한다. **공허한 테스트를 막는 방법으로 계속 쓸 만하다**
- **검증**: `typecheck` 0 · `vitest` **417 passed**(기존 411 + 6) · 사내판 E2E 4본 · 공개판 E2E **8 passed / skipped 0**(AGENTS.md 6절 — `sdk`를 건드렸다)
...
막힌 지점: **`main` 병합 동의 1건.** (앞서 적었던 "결정 4건 / 비용 차 한 자릿수 배"는 증상 확인으로 해소됐고, 이어진 결정 2건도 받았다.)

> **⚠️ #99를 구현할 때: 렌더 폭을 `targetDevice`로 바꾸는 순간 깨진다.** 스냅샷에는 캡처 폭에서
> 매치되던 CSS만 남아 있고(`removeUnusedStyles`) 숨겨져 있던 요소는 DOM에 없다(`removeHiddenElements`).
> 폭을 바꾸면 모바일로 리플로우되는 게 아니라 **데스크톱 배치가 눌려 깨진다**(실측: `column` 전환 0건).
> 검증 스크립트는 저장소에 넣지 않았다(일회성) — 절차는 이슈 #99 코멘트에 있다.

### 2026-08-06 — #90 README 스크린샷 재촬영 (4장) — 이슈 진단 정정 포함

완료:
- **#90 검토에서 이슈 본문의 진단 두 곳이 틀린 것을 실측으로 확인했다.**
  - **1번째(연결 코드 발급)는 노후가 아니었다.** 이슈가 근거로 든 **#43(확장 ZIP 배포)·#85(생성 폼 `<details>` 접힘)은 둘 다 공개판 전용 변경**이다 — `apps/web/lib/extension-release.ts`·`apps/web/public/download/`·`apps/web/components/console-home.tsx:367`. 사내판 `packages/server/src/routes/console.ts:911`은 지금도 `설치(packages/extension/dist)`이고 생성 폼은 펼쳐진 채 목록 위에 있다. `console.ts` diff·`THEME_TOKENS` 라이트 값(옛 `shell.ts` 리터럴과 동일, `--c-faint` 삭제만)·실제 렌더 캡처가 모두 일치했다. **"README는 사내판 기준"이라고 스스로 적어 놓고 공개판 변경을 근거로 삼은 자기모순이었다**
  - **"2·5번째는 유효"가 뒤집혔다.** 4번째는 T87-13 이전에 이미 `d5f2d44`(2026-07-16, **동결→캡처** 용어 전면 교체 — 스크린샷의 `✓ 동결됨`은 지금 없는 문구)·#38(페이지 헤더 밴드)·#31(SPA 배너)로 낡았고, 5번째는 `f464c73`(#86)이 **3컬럼→2컬럼**으로 갈아엎고 T87-14가 팔레트를 입혔다. 스크린샷의 잘린 캡처(`001 대시보드 메인`·`naHub`)가 바로 #86이 고친 결함이다
  - **무변경은 2번째(확장 팝업) 하나뿐.** 2026-07-17 이후 `popup.html`·`popup.css`·`popup.ts` diff 0건, `binding.ts`는 주석 1줄뿐
- **사용자 방침(2026-08-06): 판 구분 없이 낡은 것은 모두 교체. 2번째만 남기고 1·3·4·5를 새로 찍었다.** 4번째의 대상 화면은 **샘플 목업**으로 지정받았다(원본은 사내 시스템 위 컷이라 재현 불가)
- **재촬영 방법 — 재현 가능하게 남긴다.** `MOCKSPEC_DATA_DIR`을 스크래치로 돌려 **사용자 `data/`는 건드리지 않았다**(전후 7개 유지). `/sample` 라우트의 데모 HTML 3종(`LOGIN`·`ORDERS`·`DETAIL`)을 뽑아 목업 zip으로 만들어 업로드 → SDK로 화면 3개 등록·어노테이션 7개·이동 3건 지정 → `POST /api/projects/:id/export`로 산출물을 뽑아 촬영. URL 프록시 행은 로컬 오리진(`:4501`) + `MOCKSPEC_PROXY_ALLOWLIST`로 실제 등록해 만들었다
- **연결 코드는 DOM에서 마스킹한 뒤 찍었다** — 원본도 같은 처리였고, 코드에는 마스킹이 없다(프로젝트 토큰이라 평문 공개 금지)
- **README 산문도 함께 고쳤다 — 이미지만 갈면 본문이 계속 거짓말을 한다.** 5번째 설명의 **"화면 네비"** 는 #86이 없앤 좌측 사이드바를 가리키고 있었다(지금은 하단 페이저). 4번째 설명에 페이지 헤더(#38)·이동 대상 지정을 넣고, 절 첫머리에 **"아래 화면은 전부 사내판 기준"** 을 명시했다(원래 어느 판인지 표기가 없어 섞여 보였다)

다음 할 일:
- ~~브랜치 병합~~ → 사용자가 "리드미만 수정했다면 PR 없이 머지한다"고 지시해 `main`에 ff 병합·푸시하고 브랜치를 지웠다
- ~~이슈 #90 본문 정정~~ → 정정 후 **종결**했다 (원문은 `<details>`로 접어 보존)
- 다음: `#99`(대상 기기) → `#57`(마이그레이션 통합)

막힌 지점: 없음.

> **⚠️ 스크린샷은 반드시 `deviceScaleFactor: 2`로 찍는다 (2026-08-06, 사용자 지적 "흐리다").**
> 1차 재촬영은 브라우저 확장 도구로 찍었는데, **그 경로는 CSS 픽셀(=1x)로 떨어진다** —
> 원본이 2x 레티나였으므로 그대로 교체하면 눈에 띄게 흐려진다. "GitHub 표시 폭에서는
> 충분하다"고 판단했던 것이 틀렸다. **Playwright(`chromium.launch` → `newContext({ deviceScaleFactor: 2 })`)로
> 다시 찍어 2880px 폭을 얻었다.** 확장 도구의 `zoom` 액션은 업스케일(보간)이라 해상도가
> 늘지 않는다 — 대안이 아니다.
>
> 촬영 스크립트는 저장소에 넣지 않았다(일회성 도구). 절차는 위 "재촬영 방법"과 이슈 #90 본문에 있다.
> 함정 둘: ① 연결 코드 결과 패널이 뜨면 페이지가 **자동 스크롤**되므로 클립 전에 `scrollTo(0,0)`으로
> 되돌려야 헤더가 담긴다 ② 흐름도 토글 셀렉터는 `.ms-flow .ms-collapse-btn`이고 **기본 접힘**이라
> (킥오프 19차) 펼치지 않으면 캡션이 약속한 흐름도가 안 나온다

### 2026-08-06 — PR #100·#101 병합 — #97 종결, 네 표면이 모두 THEME_TOKENS를 쓴다

완료:
- **PR #100 병합 (`7a0196b`) — #97 종결.** 공개판 E2E 수동 실행이 AGENTS.md 6절 "병합 전 검증"이 됐다.
  트리거는 고정 목록이 아니라 **도출 명령**이다(`apps/web`이 끌어오는 패키지를 뽑는다)
- **PR #101 병합 (`54ece29`) — T87-14 완료.** 산출물이 제품 팔레트로 들어와 **네 표면(공개판·사내판·SDK·산출물)이
  모두 `THEME_TOKENS`를 단일 원천으로 쓴다.** M-01·M-02·M-08이 닫혔고 F-03이 네 표면 전부에 깔렸다
- 병합 후 정리 완료 — 원격·로컬 토픽 브랜치 삭제, `main` 동기화. 상시 워크트리 없음(종전과 같다)

검증: 병합된 `main`에서 `typecheck` 0 에러 · `vitest` **365 passed**.
**프로덕션 반영 확인** — `draftify-html.vercel.app/sample`의 `<style>`이 9,461B → **13,122B**(로컬
실측과 일치)이고, 토큰 선언(`--c-accent`·`--c-btn-bg`)·`:focus-visible`·`.ms-flow-arrow`·Pretendard가
모두 있으며 **옛 팔레트(`#1a73e8`·`#5f6368`·`#202124`·헤더 그라디언트)는 전부 사라졌다.** 의도적으로
남긴 경고색 2종만 있고, 뷰어 번들의 화살촉도 `class` 속성으로 바뀌어 인라인 `fill`이 없다.

> **⚠️ 병합 직후의 프로덕션 확인은 이른 판정이 되기 쉽다.** 머지 1분 뒤 curl에서는 아직 이전
> 빌드(9,461B)가 내려왔다. 게다가 `ms-flow-arrow`가 잡혀 "일부만 반영됐다"고 오독할 뻔했는데,
> 그건 **원래 있던 marker `id`**였다(내가 추가한 것은 `class`). **문자열 존재만 보지 말고
> 어디에 있는지(style 블록 안/밖)까지 본다.**

**⚠️ 봇 날짜 지적이 이 세션에만 네 번 왔다(UTC 기준). 근거는 위 세션 로그에 적어 뒀으니 재사용한다.**

다음 할 일:
1. **#90 README 스크린샷 5장** — 이제 산출물을 더 바꿀 예정이 없어 **다시 찍을 일이 없는 시점**이다
2. **#99 대상 기기** — `Scene` 필드 추가라 AGENTS.md 4절 절차가 선행한다
3. **#57 마이그레이션 통합** — 끝나면 **#97 A안(테스트 전용 Supabase로 CI 자동화) 재검토 조건**이 된다

막힌 지점: 없음

### 2026-08-06 — 봇 리뷰 반영: 화살촉 드리프트·SDK 트리거 누락 (PR #100·#101)

완료:
- **PR #101 — 흐름도 화살촉이 옛 팔레트에 남아 있던 것을 고쳤다 (Codex P3).** `export.ts`의 CSS만
  토큰으로 옮기고 `packages/viewer/src/main.ts`의 인라인 `fill="#5f6368"`을 놓쳐, **간선은 슬레이트인데
  화살촉만 Google 그레이**로 남아 있었다. 클래스(`.ms-flow-arrow`) + `VIEWER_CSS`에서 같은 `--c-muted`.
  실측 확인: 화살촉 fill = 간선 stroke = `rgb(100,116,139)`, 산출물 HTML에 `#5f6368` 0건
- **PR #101 — 고정 개수 표기 정정 (Codex P2).** `27종` → `THEME_TOKENS.light 전부`. 팔레트는 자랄 것이
  예정돼 있다(4.1절의 warn 토큰 공백). 6.6절의 과거 측정 기록은 자라지 않으므로 그대로 뒀다
- **PR #100 — 공개판 E2E 트리거에 `packages/sdk`가 빠져 있었다 (Codex P2).** `apps/web/lib/sdk-bundle.ts`가
  `sdk/dist/sdk.js`를 빌드 시점에 인라인하고 공개판 스위트가 그 결과를 단정한다. **고정 목록 대신
  도출 명령을 규약으로** 바꿨다 — 손으로 적은 목록이 조용히 낡은 사례가 바로 이것이었다

검증: `typecheck` · `vitest` **365 passed** · 사내판 E2E **4 passed** · **공개판 E2E 전본 통과(skipped 0)**.
화살촉이 두 배포 공유 코드라 공개판도 다시 돌렸다 — **PR #100이 만든 그 트리거가 곧바로 값을 했다.**

**⚠️ "한 파일만 고치면 되는 변경"이라는 판단을 실측으로 검증하지 않았다.** 초기 조사에서
`packages/viewer/src/main.ts -> 1`(색 리터럴 1건)을 **눈으로 보고도 열어보지 않았다.** 파일별 개수만
세고 내용을 안 본 것이 원인이다. **grep이 0이 아니면 반드시 연다.**

다음 할 일: 아래 항목과 같다 (PR 두 건 병합 동의 대기).

막힌 지점: 없음

### 2026-08-06 — #97 방향 확정(C안)과 T87-14 산출물 시각 언어 — PR #100·#101 열림

완료:
- **#97을 C안(수동 실행 규약화)으로 확정하고 AGENTS.md 6절에 명문화했다 (PR #100).** 사용자 결정.
  A안(테스트 전용 Supabase)은 **#57이 선행해야 비용이 내려가서**, B안(프로덕션 DB + 정리 강제)은
  **레이트리밋 카운터가 CI 트래픽으로 오염돼서** 미뤘다. 봇 리뷰 3건 처리 — 2건 반영(hasEnv 실제
  조건 명시 / 하드코딩된 개수 제거), **1건 기각**(날짜)
- **T87-14 산출물 시각 언어 (PR #101).** `export.ts`가 `THEME_TOKENS`를 빌드타임 import해
  `:root`에 라이트 토큰 27종을 찍고 색을 전부 `var(--c-*)`로 바꿨다. 강조색 `#1a73e8`→`--c-accent`,
  Google 그레이 8종→slate, 헤더 그라디언트 제거(M-08), 반경 4·6·8→4·8, **`:focus-visible` 링 신설**(F-03).
  **네 표면이 모두 같은 원천을 쓰게 됐다** — M-01·M-02·M-08이 이 작업으로 닫힌다
- **AGENTS.md 4절 절차를 밟았다** — 킥오프 20차(`ee52cfc`) → 문서 동기화(`4c0eb4a`) → 구현(`dce1eda`)

검증:
- `typecheck` · `vitest` **365 passed** · 사내판 E2E **4 passed** · **공개판 E2E 8 passed**(방금 만든
  규약대로 손으로 돌렸다 — `export.ts`는 두 배포가 공유한다)
- 실제 산출물(`/sample`)을 Chrome에 띄워 computed 값으로 확인. **Tab 순회 9개 정지점 전부에 링이
  있다**(링 없는 정지점 0). 대비 전부 AA 통과(본문 17.85 · 채움 위 흰 글자 6.29 · 포커스 링 4.47:1)
- 외부 참조 **실참조 0건**. 레이아웃 불변(화면영역 1170px · 디스크립션 300px · 가로 오버플로 0)

**⚠️ `outline`은 슬롯이 하나뿐이다 — 상태 두 개가 같은 슬롯을 노리면 하나가 조용히 사라진다.**
활성 마커(앰버 outline)에 포커스 링도 outline으로 주면 활성 표시가 포커스 중에 없어진다.
마커만 `box-shadow`로 전달해 둘을 공존시켰고, **`.is-active`를 focus 규칙 뒤에 뒀다**(특정도가
같아 순서가 승부를 가른다). **링이 보이므로 "포커스는 되네"로 넘어가기 쉬운 부류의 결함이다.**

**⚠️ 새 토큰을 만들기 전에 제품에 이미 있는 조합을 찾는다.** 연한 강조 3종(`#e8f0fe`·`#d2e3fc`·`#9ec1f7`)에
대응하는 토큰이 팔레트에 없어 `--c-accent-bg` 같은 것을 만들 뻔했다. 그러나 SDK `.btn`과 콘솔
`.g-badge.is-primary`가 이미 `테두리 --c-accent · 배경 --c-accent-ring · 글자 --c-accent`를 쓰고 있었다.
**토큰을 새로 만들면 네 표면과 파리티 테스트를 동시에 건드린다** — 4.1절이 warn 토큰을 별건으로
미룬 것과 같은 이유다.

**⚠️ 산출물 CSS는 산출물 하나하나에 통째로 실린다.** 초안의 긴 주석이 `<style>`을 +4,499B 늘렸다.
근거는 `ui-standard.md` 6.8절에 두고 코드에는 포인터만 남겨 +3,514B로 줄였다. **이 파일에서는
"주석을 넉넉히"가 기본값이 아니다.**

**⚠️ 봇의 날짜 지적은 UTC 기준이다 — 이번 세션에만 네 번 왔고 전부 기각했다.** Codex·CodeRabbit이
PR #100·#101 양쪽에서 "2026-08-06은 미래"라고 했으나, 커밋은 전부 KST 자정 이후다
(`ee52cfc` `2026-08-06 00:22 +0900` = UTC 8월 5일 15:22). 이 저장소는 **로컬(KST) 날짜**로 기록해
왔다 — 직전 커밋 `4948a0f`도 `23:35 +0900`에 "2026-08-05"로 적혔고, UTC였다면 우연히 같아 보일
뿐이라 **자정을 넘긴 작업에서만 규칙이 드러난다.** 같은 부류를 기각한 선례도 있다(PR #33).
**자정 직후에 작업하면 반드시 다시 온다 — 근거를 한 번 적어 두고 재사용한다.**

다음 할 일:
1. **PR #100·#101의 남은 봇 리뷰 확인 후 병합 여부를 사용자에게 확인** (AGENTS.md 6절 — 건별 동의).
   CodeRabbit이 PR #101에서 리뷰 한도에 걸려 있었다(`@coderabbitai review`로 재요청 가능).
   **병합 직전에 `--json reviews`와 `pulls/{n}/comments`를 한 번 더 본다**(PR #92의 23초 사례)
2. **#90 README 스크린샷 5장** — **#101 병합 후에 찍는다.** 지금 찍으면 산출물 컷을 두 번 찍게 된다
3. **#99 대상 기기** — `Scene` 필드 추가라 AGENTS.md 4절 절차가 선행한다

막힌 지점: 없음 (병합 동의만 대기)


### 2026-08-05 — PR #98 병합 (`8dcb9b7`) — #86 종결, 산출물 화면기획서 2컬럼 개편 (킥오프 19차)

완료:
- **PR #98 병합 — 산출물이 타이틀 + 화면영역 + 디스크립션 2컬럼이 됐다.** 5커밋·15파일(+369/−158). **프로덕션 반영까지 확인**(`draftify-html.vercel.app/sample`에 `ms-scene-nav`·`ms-stage-scale`·`flowCollapsed: true` 존재, `ms-scene-button` 0건). 병합 후 브랜치 삭제·`main` 동기화·리뷰 스레드 4건 resolve 완료
- **이슈 #99 신규 등록** — 대상 기기(모바일 기획인데 PC 폭이 굳는 문제). #86 범위 밖으로 분리
- **AGENTS.md 4절 절차를 밟았다** — 문서 개정(`f5b6008`)이 구현(`f464c73`)보다 앞선다

**⚠️ 사용자가 방향을 세 번 되돌렸다. 실측만으로는 옳은 구조가 나오지 않았다.**

내가 처음 낸 세 안(축소만 / 축소+흐름도 접힘 / 흐름도를 좌측 열로)은 **전부 좌측 사이드바를
남기는 전제**였고 전부 기각됐다. 사용자 지적: *"보통의 화면기획서는 타이틀, 화면영역과
디스크립션 영역으로 나눠진다. **화면영역 좌측에 메뉴 스텝을 배치하지 않는다.**"* 실측은
"사이드바가 73~83% 비었다"까지 말해 줬지만 **"그 열이 애초에 없어야 한다"는 도메인 관례는
측정에서 나오지 않았다.** 다음에 산출물 형태를 다룰 때는 측정보다 **실제 화면기획서 관례**를
먼저 확인한다.

**⚠️ 이슈 진단이 세 곳 어긋나 있었다 (#93·#91에 이어 세 번째다).**

| 이슈 서술 | 실측 |
|---|---|
| 세로 278px 넘침이 핵심 | **가로가 지배적** — 1280×800에서 가로 534px(세로의 2.4배), 로그인 폼이 반토막 |
| "흐름도를 접으면 104px 회수" | **68px**(헤드 37px은 남는다). 게다가 접어도 넘침 279→211px, **가로엔 무효**(344→344) |
| 좌측·우측이 비슷하게 비었다 | 사이드바 **73~83%** vs 패널 22~50%. 폭도 275px이 아니라 300px |

**⚠️ 실측이 아니었으면 놓쳤을 구현 결함 2건.** ① 좁은 화면(≤900px) 스택 모드는 페이지
스크롤로 돌아가는데 높이를 제약으로 써서 배율이 **0.217**까지 떨어졌다(캡처 278px로 축소).
② **`offsetTop`은 `offsetParent` 기준**이라 `.ms-main-body`가 static이면 헤더·흐름도 높이가
딸려와 이중으로 빠졌다(1470px에서 배율 0.706 대신 0.571). **예측치와 실측치의 불일치가
②를 드러냈다** — 숫자가 예상과 다르면 그 자체가 단서다.

**⚠️ #97에서 내가 지적한 공백에 내가 그대로 빠졌다.** 공개판 E2E가 `.ms-scene-button`을
단정한 채 깨져 있었는데, `test:e2e:web`이 CI에서 안 돌아 사내판 4본만 보고 통과로 판단했다.
**Codex P2가 잡았다.** 앞 PR에서 이슈까지 등록해 놓고 다음 PR에서 같은 함정을 밟았다 —
**공개판 코드를 건드리면 `npm run test:e2e:web`을 손으로 돌린다**(맨 위 ⚠️ 항목).

**리뷰:** CodeRabbit 2건 + **Codex 4라운드(P2 2건·P3 2건)**. 전부 반영, 4라운드째 지적 0건.
Codex가 잡은 것 중 둘은 문서·제품 문구였다 — ⓐ 킥오프 19차가 축소 대상을 `.ms-stage-wrap`으로
적어 **방금 고친 클리핑 경로를 가리키고 있었다** ⓑ `/guide` 페이지가 사용자에게 "3단 구성"을
안내하고 PRD **FR-EXP-03이 P0 요구사항으로 사이드바를 요구**하고 있었다(4절 동기화에서 PRD와
제품 문구를 빠뜨렸다). **계약 문서를 고칠 때 `docs/`만 훑지 말고 PRD·제품 안내 문구까지 본다.**

검증: vitest **365 passed**(+5), 사내판 E2E **4본**, **공개판 E2E 8본**, typecheck green.
`/sample` 실측 — 배율 1470=0.681 / 1280=0.739 / 1920=1.000 / 900=0.677, **넘침 0, 마커 오차 1px**.
마커 정합은 iframe **문서** 좌표계가 부모 transform에 영향받지 않는 성질로 지켰다 — 마커 수식은 한 줄도 안 고쳤다.

다음 할 일:
- **#90 README 스크린샷** — #86으로 산출물 컷까지 노후화됐으니 5장을 한 번에 찍는다
- **#97 CI 공백** — A/B/C 택일이 사용자 결정이라 그 답이 먼저다
- **T87-14 산출물 시각 언어** — **#86이 끝나 착수 조건 충족.** `THEME_TOKENS` 빌드타임 재사용(`ui-standard.md` 0절 정정)
- **#99 대상 기기** — 4절 절차 선행

막힌 지점:
- **#97의 선택지 택일 — 사용자 결정 필요** (A: 테스트 전용 Supabase 권고)

### 2026-08-05 — PR #96 병합 (`f71bb74`) — #91 종결, 공개판 E2E 타임아웃 + CI 공백 발견(#97)

완료:
- **PR #96 병합 — 공개판 E2E 8본이 다시 전부 통과한다.** 1커밋·1파일(+17), 테스트 코드만. 제품 코드는 건드리지 않았다. 병합 후 원격·로컬 브랜치 삭제와 `main` 동기화까지 마쳤다. `Closes #91`로 이슈 자동 종결
- **이슈 #91에 원인 정정 코멘트 등록**([#issuecomment-5190348051](https://github.com/landfill/draftify-html/issues/91#issuecomment-5190348051)) — 본문에 박힌 사실오류 2건을 종결 전에 바로잡았다
- **이슈 #97 신규 등록** — CI 공백. 아래 참조
- 이 항목은 **병합 후 `main`에 직접 커밋했다** (PR #94에서 세운 원칙)

**⚠️ 이슈 본문의 관찰 두 개가 틀려 있었다 — 스냅샷을 그대로 믿었다면 엉뚱한 곳을 팠다.**

이슈 #91은 *"탭이 존재하고 보이는 상태다 → actionability 대기가 풀리지 않는다"* 로 진단하고
**`trace.zip`을 열어 보라**고 남겼다. 그러나 `error-context.md`를 다시 읽으니 `<details>`
안에 **summary 하나뿐이고 `+ 열기`가 붙어 있다** — 패널은 **접혀 있었다.** 본문이 인용한
`tab "ZIP 업로드" [ref=e28]`은 그 스냅샷에 없다(e28은 `QA · 화면 0개 …` 메타 줄이다).
탭은 닫힌 `<details>` 안이라 렌더조차 안 됐다. **`trace.zip`은 열 필요가 없었다.**

원인은 본문의 **가설 ①이 맞았다** — `console-home.tsx:96-108`이 프로젝트 수 0↔1 경계에서
생성 패널을 자동으로 접는다(#85). 테스트가 프로젝트를 만든 뒤(0→1 → 접힘) 그 안의 탭을
곧바로 누르니 180초를 대기했다. **PR #88이 같은 부류를 767행에서 이미 고쳤는데
(`PR #88 Codex P2`) 879행만 빠져 있었다.** 767행을 그대로 복사하면 되는 일이었다.

교훈: **Playwright 스냅샷의 "요소가 보인다"를 사람이 요약한 문장으로 믿지 않는다.** 원문
YAML에서 그 ref가 실제로 있는지 확인한다. 이 이슈는 등록자가 다른 스냅샷을 본 듯하다.

**⚠️ "CI에서는 통과했다"도 사실이 아니었다 — CI가 이 스위트를 돌린 적이 없다 → #97.**

이슈는 *"PR #88·#89의 CI에서는 통과했으니 환경 차이인가"* 로 조사 방향을 한 번 돌렸다.
확인해 보니 `.github/workflows/ci.yml:45`는 `npm run test:e2e`(사내판)만 실행하고
**`test:e2e:web`은 `ci.yml`·`pr-agent.yml` 어디에도 없다.** 환경 차이가 아니라 #85가
들여온 실제 회귀였고, PR #88(8/1)부터 다음 날 우연히 발견될 때까지 살아남은 이유가 그
공백이다. **그대로 CI에 넣을 수는 없다** — 자세한 제약·선택지는 #97과 위 "열린 이슈" 참조.

**리뷰:** CodeRabbit 0건 · PR-Agent 0건 · **Codex 0건**. 다만 **Codex는 자동으로 붙지
않아 `@codex review`를 손으로 걸어야 했다**(위 시작점의 ⚠️ 항목). 걸기 전 8분간 무반응이었고,
걸자 1분 58초 만에 회신했다.

검증: `test:e2e:web` **8/8 통과(33.7s)** — 문제 스펙은 180초 타임아웃 → **5.0s**. `typecheck` green. CI `verify` pass.

다음 할 일:
- **#86 산출물 UI** 착수가 우선. 착수 전 ⓐ 구조까지 바꾸는지 판정(바꾸면 AGENTS.md 4절 절차 선행 — `output-standard.md`·`s1-kickoff-spec.md`가 계약) ⓑ `ui-standard.md` 3.0절·6.5절 필독
- **#90 README 스크린샷**은 #86 뒤가 낫다 — #86이 5번째(산출물) 컷도 노후화시키므로 5장을 한 번에 찍는다
- **#97**은 A/B/C 택일이 사용자 결정이라 그 답이 먼저다

막힌 지점:
- **#97의 선택지 택일 — 사용자 결정 필요.** A(테스트 전용 Supabase) 권고. 그때까지 공개판 작업은 `test:e2e:web`을 **로컬에서 손으로** 돌린다
- **정리 대기 브랜치 2개** — 로컬 `landfill/issue-85`, 원격 `origin/landfill/ui`. 둘 다 `main`에 병합 완료·고유 커밋 0건이라 지워도 잃을 것이 없으나, 이 세션이 만든 것이 아니라 삭제 동의를 받지 못했다

### 2026-08-03 — PR #95 병합 (`0eeea4b`) — #93 종결, 좁은 화면 목록 접힘

완료:
- **PR #95 병합 — 두 판의 콘솔 목록이 720px 이하에서 세로로 접힌다.** 2커밋·3파일. 병합 후 원격·로컬 브랜치 삭제와 `main` 동기화까지 마쳤다. `Closes #93`으로 이슈 자동 종결
- 이 항목은 **병합 후 `main`에 직접 커밋했다** (PR #94에서 세운 원칙 — 코드가 딸린 작업의 PROGRESS는 PR에 넣지 않는다)

**⚠️ 이슈에 적힌 진단을 그대로 구현했다면 절반만 고쳤다.**

이슈 #93은 `.c-project-actions`의 가로 오버플로(+67px @ 390px)를 지목했고 재현은 정확했다.
그러나 착수 후 실측하니 **같은 원인이 만든 더 큰 피해가 따로 있었다** — `.c-project-info`가
`flex: 1 1 0` + `min-width: 0`이라 버튼에 밀려 **0px까지 눌리고, 프로젝트 이름이 사라진다.**

| 폭 | 사내판 오버플로 | 사내판 info | 공개판 오버플로 | 공개판 info |
|---|---|---|---|---|
| 320px | +137 → **0** | 0 → **181** | 0 → 0 | **0** → **182** |
| 390px | +67 → **0** | 0 → **252** | 0 → 0 | **9** → **252** |
| 480px | **0** → 0 | **0** → **342** | 0 → 0 | 99 → **342** |
| 1280px | 0 → 0 | 554 (무변화) | 0 → 0 | 699 (무변화) |

**여기서 두 가지가 갈렸다:**

1. **브레이크포인트가 480px이 아니라 720px이 됐다.** 넘침만 보면 480px에서 이미 0이라 거기서
   접으면 충분해 보인다. 그러나 그 폭에서 info는 **0px**이었다 — 넘침이 멈춘 것은 정보가 다
   눌려 사라진 뒤였을 뿐이다. 720px은 두 판이 이미 쓰던 지점이라 접는 폭도 하나로 유지된다
2. **"공개판은 0이라 괜찮다"가 뒤집혔다.** 넘치지 않은 이유는 버튼이 2~3개뿐이어서(사내판
   5개)지 결함이 없어서가 아니었다. 원인 CSS는 양쪽에 복제돼 있어 한쪽만 고치면 접히는 폭이
   배포마다 갈린다 — 포커스 링을 공개판까지 넣은 **T87-12와 같은 판단**으로 함께 고쳤다

> **그래서 규약에 넣었다 (`ui-standard.md` 7절): 가로 오버플로 0을 반응형 통과로 읽지 않는다.**
> 눌려 사라지는 요소는 그 지표에 잡히지 않으므로 **주요 정보 요소의 실제 렌더 폭을 함께 찍는다.**
> 브레이크포인트를 건드렸으면 **경계의 양쪽**(720·721px)을 찍는 것도 함께 넣었다.

**리뷰 2건 — 둘 다 반영 (`b6fdcd6`):**

| 리뷰 | 지적 | 처리 |
|---|---|---|
| Codex P2 | F-05를 "해소"로 표기하면서 같은 절의 "`console.ts`에 `@media` 0건"과 1.1절 인벤토리의 "720px (`shell.ts`만)"을 그대로 뒀다 | 반영 — 인벤토리를 고치고 원 서술에는 시점 표시 + 정본 위치 명시 |
| CodeRabbit Minor | 7절 검증 기준에 720·721px이 없어 경계 양쪽을 확인하지 않는다 | 반영 — 기준에 추가, 6.7절 표의 721px 행 보충 |

**Codex 지적이 이 문서 부류의 함정을 정확히 짚었다**: 해소 표기만 하고 현재 상태 서술을 남기면
`ui-standard.md`가 인벤토리의 기준이므로 **다음 UI 작업이 "console.ts에는 반응형 규칙이 없다"를
사실로 읽는다.** 원 서술 자체는 지우지 않고 시점을 달았다 — 확인 경로를 남기는 편이 낫다.

- 검증: `typecheck` · `vitest` **360 passed** · 사내판 E2E **4 passed** · 공개판 E2E **7 passed** · CI 6체크 green
- 12개 폭(320~2400px)에서 가로 오버플로 **전 구간 0**, 라이트·다크 동일. 경계는 **720/721에서 정확히 전환**
- 공개판 실측은 콘솔 목록이 로그인 없이 렌더되지 않아, E2E와 같은 경로(admin `generateLink` → `/auth/confirm`)로 **임시 계정을 만들어 측정하고 삭제**했다

다음 할 일:
1. **열린 이슈에서 골라 시작한다.** 권장 순서 — **#86**(산출물 UI. 이것이 끝나야 `ui-standard.md`의 산출물 항목을 착수할 수 있다) → **#90**(README 스크린샷 1·3·4번째) → **#91**(공개판 E2E 타임아웃)
2. **#86에 착수하면 AGENTS.md 4절 절차가 선행한다** — 산출물의 섹션 구조·ID 체계는 `output-standard.md`·`s1-kickoff-spec.md`의 계약이라, 레이아웃을 넘어서면 킥오프 문서를 먼저 고쳐야 한다
3. **#91을 잡으려면 `trace.zip`부터 연다.** 이번 세션에서 `origin/main` 상태(변경을 `git stash`로 걷어낸 상태)로 **같은 지점·같은 증상 재현을 다시 확인**했다 — 여전히 원인 미확인이고, 스냅샷상 탭은 보이는데 클릭 actionability가 풀리지 않는다

막힌 지점:
- **진행 중인 브랜치·PR이 없다.** 사용자 결정을 기다리는 것도 없다
- **#91** 공개판 E2E `경로 D 콘솔` 타임아웃 — 원인 미확인 (위 3번)
- **#46**(매직링크 크로스디바이스)은 종전대로 SMTP 제약으로 사용자 결정 대기
- 로컬에 **`landfill/issue-85` 브랜치가 남아 있다** — `main`에 완전히 병합돼 미병합 커밋 0건인 죽은 브랜치다. 이번 작업 범위 밖이라 지우지 않았다

### 2026-08-02 — PR #94 병합 (`255e9cb`) — #87 후속까지 종료

완료:
- **PR #94 병합.** PR #92의 Codex 3차 지적(미작성 마커의 포커스 링이 `opacity`에 흐려져 2.48:1)을 반영한 CSS 한 줄 + 문서. 병합 후 원격·로컬 브랜치 삭제와 `main` 동기화까지 마쳤다
- **이 항목은 `main`에 직접 커밋했다** — 바로 아래 교훈을 그대로 적용한 첫 사례다

**⚠️ 이 PR에서 배운 것 — 프로세스 문서를 PR에 넣으면 리뷰가 그쪽으로 쏠린다.**
PR #94의 실질 변경은 **CSS 한 줄**이었는데, 봇 지적 4건이 **전부 `docs/PROGRESS.md` 산문**이었다
(핸드오프 필드 누락 P1 · 문서 내 모순 P1 · 개수 표기 P1 · 상대 시간 표현 Minor).
셋은 타당해 반영했지만, **정작 그 CSS 한 줄은 아무도 다시 보지 않았다.**
고칠 때마다 새 문장이 생겨 다음 라운드 대상이 되는 구조라 PR #89에서 8라운드를 돈 것과 같은 함정이다.
→ **사용자 지시로 원칙을 세웠다: `docs/PROGRESS.md`만 바뀐 커밋에는 `@codex review`를 걸지 않는다.**
AGENTS.md 6절이 이 파일을 "리뷰 대상이 아니라 다음 세션이 읽을 상태 기록"으로 규정하고
`main` 직접 커밋 예외를 둔 이유가 정확히 그것이다.

- 검증(PR #94): `typecheck` · `vitest` **360 passed** · 빌드 2종 · 사내판 E2E **4 passed** · CI 전부 success
- 대비 실측: 미작성 마커 포커스 시 `opacity` 0.65 → 1, 링 실효 색 `#9a9cf6` → `#6366f1`, **2.48:1 → 4.47:1**

다음 할 일:
1. **열린 이슈에서 골라 시작한다.** 권장 순서 — **#86**(산출물 UI. 이것이 끝나야 `ui-standard.md`의 산출물 항목을 착수할 수 있다) → **#93**(사내판 콘솔 390px 넘침) → **#90**(README 스크린샷 1·3·4번째) → **#91**(공개판 E2E 타임아웃, 원인 미확인)
2. **#86에 착수하면 AGENTS.md 4절 절차가 선행한다** — 산출물의 섹션 구조·ID 체계는 `output-standard.md`·`s1-kickoff-spec.md`의 계약이라, 레이아웃을 넘어서면 킥오프 문서를 먼저 고쳐야 한다
3. UI 작업 전 `docs/ui-standard.md` **3.0절(기각 이력)** 과 **6.5절(포커스 링 함정)** 을 읽는다

막힌 지점:
- **진행 중인 브랜치·PR이 없다.** 사용자 결정을 기다리는 것도 없다
- **#91** 공개판 E2E `경로 D 콘솔` 타임아웃 — `origin/main`에서도 재현되나 원인 미확인. CI에서는 통과한 이력이 있어 환경 차이인지 실제 회귀인지 갈리지 않았다
- **#46**(매직링크 크로스디바이스)은 종전대로 SMTP 제약으로 사용자 결정 대기

### 2026-08-02 — #87 종결 (PR #92 병합 `aa839cb`) + Codex 3차 후속

완료:
- **PR #92 병합 — T87-10~13이 프로덕션에 나갔다.** 11커밋·10파일. 브랜치 4개를 사용자 요청으로 하나로 합쳐 올렸고(`fix/ui-standard-remaining-87`), 커밋 단위로는 성격이 나뉜 채다. 병합 후 원격·로컬 브랜치 4개 삭제, `main` 동기화까지 완료
- `Closes #87`로 **이슈 #87 자동 종결**

**리뷰 결과 — Codex 3건, PR Agent 2건(둘 다 기각):**

| 리뷰 | 지적 | 처리 |
|---|---|---|
| Codex 1차 | `.seg button` 링이 `overflow: hidden`에 잘림 | 반영 (`3e18a0b`) |
| Codex 2차 | 안쪽 링이 활성 배경 위에서 1.41:1 | 반영 (`682572a`) |
| Codex 3차 | `.marker--empty`의 `opacity`가 링까지 흐리게 (2.48:1) | **병합 후 후속 PR** |
| PR Agent | `theme.parity.test.ts` 경로 계산 | 기각 — 실측 반박 (4단계는 저장소 밖) |
| PR Agent | 주석의 "장면" 용어 | 기각 — 이슈 #2 범위 밖 + AGENTS.md 7절 선례 |

**⚠️ Codex 지적 3건 중 2건이 "내 직전 수정이 만든 새 결함"이었다.**
1차(잘림)를 `-2px`로 고치자 2차(활성 배경 위 대비)가 생겼고, T87-12에서 링을 넣은 것 자체가
3차(`opacity` 합성)를 만들었다. **포커스 링처럼 전 표면에 까는 규칙은 한 번에 끝나지 않는다** —
링이 놓이는 자리마다 배경·투명도·클리핑이 다르기 때문이다.

**⚠️ 3차 지적이 병합과 23초 차로 엇갈렸다.** `2026-08-02 14:50:28` 지적 → `14:50:51` 병합. 반영할 틈이 없었다.
**`gh pr merge` 직전에 리뷰 채널을 마지막으로 한 번 더 확인할 것.**
2차 지적도 하마터면 놓칠 뻔했는데(사용자가 "피드백 다시 체크"로 잡아 줬다), 원인은
**Codex 리뷰가 `gh pr view --json comments`에 안 잡히기 때문**이다 — `--json reviews`와
`gh api repos/{owner}/{repo}/pulls/{n}/comments`를 함께 봐야 한다.

**후속 PR (진행 중)** — 브랜치 `fix/empty-marker-focus-opacity`:
- `.marker--empty { opacity: .65 }`가 **`outline`·`box-shadow`를 포함한 요소 전체**를 합성해,
  미작성 핀에 포커스가 가면 링이 흰 목업 위에서 `#9a9cf6`로 렌더돼 **2.48:1**이었다
  (계산과 실측이 일치). 포커스 동안만 `opacity: 1`로 돌려 **4.47:1** 회복
- 트레이드오프: 그 순간에는 "미작성" 흐림 표시가 사라진다. 포커스는 일시적이고 그때는 링이
  더 강한 신호라 접근성을 우선했다. 흐림을 `background` 알파로 옮기는 대안은 흰 테두리·글자의
  겉모습까지 바꾼다
- **교훈: 투명도로 상태를 표현하는 요소에 링을 넣을 때는 링이 그 투명도에 딸려가는지 본다.**
  `opacity`·`filter`가 그런 속성이고, **색만 계산해서는 안 잡힌다 — 합성 후 실효 색으로 재야 한다**

다음 할 일:
1. **PR #94 리뷰 대응·병합.** 병합은 건별 사용자 동의가 필요하다(AGENTS.md 6절). 병합 뒤에는 원격·로컬 브랜치 삭제와 `main` 동기화까지 6절 "병합 후 정리"대로 한다
2. 열린 이슈 처리 순서를 정한다 — **#86**(산출물 UI. 이것이 끝나야 `ui-standard.md`의 T87-14/산출물 항목을 착수할 수 있다) · **#93**(사내판 콘솔 390px 넘침, F-05 실측분) · **#90**(README 스크린샷 1·3번째. **4번째 SDK 패널도 T87-13으로 팔레트가 바뀌었으니 같이 찍는다**) · **#91**(공개판 E2E 타임아웃, 원인 미확인)
3. UI를 다시 건드릴 때는 `docs/ui-standard.md` **3.0절(기각 이력)** 과 **6.5절(포커스 링 함정)** 을 먼저 읽는다

막힌 지점:
- **PR #94 병합 동의 대기** (사용자 결정)
- **#91** 공개판 E2E `경로 D 콘솔` 타임아웃 — `origin/main`에서도 재현되지만 원인 미확인이고, CI에서는 통과한 이력이 있어 환경 차이인지 실제 회귀인지 갈리지 않았다
- 그 밖의 미결 결정 없음 — Q-01~Q-07이 모두 정리됐다(Q-07 히어로 미리보기 폭만 "기본값 860px로 진행" 상태)

### 2026-08-02 — T87-13 SDK 팔레트·반경 통일 (#87의 마지막 작업)

완료:
- **Q-03이 "라이트 고정"으로 결정됐다(사용자).** SDK는 목업 페이지 안에서 돌아 콘솔의 `data-theme`을 상속할 수 없고, **OS가 다크여도 밑에 깔린 목업은 대개 라이트**라 패널만 어두워지면 그 대비가 거슬린다. 그래서 `:host`에 `THEME_TOKENS.light`만 선언하고 `prefers-color-scheme`도 보지 않는다
- **T87-11의 설계가 여기서 값을 했다.** 그때 헬퍼를 "선언만 찍고 셀렉터·중괄호는 호출부에 남기도록" 만들어 둔 덕에, Shadow DOM용 `:host`에 그대로 끼워 넣을 수 있었다
- **M-01·M-02·M-03이 SDK에서 해소됐다** — 강조색 `#2f6feb`→`--c-accent`, 임의 회색 11종→slate 토큰, 반경 **7종(3·4·5·6·7·8px·10px)→3종(4·8·999px)**. T87-12가 넣었던 `#2f6feb` 리터럴도 토큰으로 바뀌었다
- **흐린 회색 8종이 `--c-muted` 하나로 접혔다** (`#666`·`#888`·`#999`·`#aaa`·`#bbb`·`#7a7a7a` 등). 4.1절이 "가장 흐린 텍스트 토큰은 `--c-muted`"라고 못박았기 때문이고, 이들은 전부 보조 텍스트라 하나로 충분했다

**⚠️ 유지하려던 값에서 기존 결함이 나왔다.** 선택 마커 `#d9480f`는 강조색과 구별하려고 그대로 두려 했는데, **흰 11px 굵은 글자와의 대비가 4.30:1로 AA 미달**이었다(큰 텍스트 예외는 18.66px부터라 해당 없음). 색상을 유지하면서 통과하는 `orange-700 #c2410c`(**5.18:1**)로 낮췄다. **"유지"로 분류한 값도 대비는 재야 한다.**

**남긴 리터럴**: warn 계열 4종(토큰 자체가 없다 — 4.1절이 이미 지적, 세 표면을 함께 건드릴 일이라 별건) · 선택 주황 · 의도적 흰색(마커 테두리·흰 띠) · 그림자 `rgba(0,0,0,…)`(M-05의 일).

**별건으로 남긴 것 하나** — `--c-border-2`(`#cbd5e1`)를 입력 테두리로 쓰면 흰 배경 위 **1.48:1**로 SC 1.4.11(3:1)에 못 미친다. 치환하며 계산하다 발견했다. **SDK만의 문제가 아니라 공개판·사내판이 이미 같은 토큰을 같은 용도로 쓰고 있어** 제품 전체의 사안이고, SDK만 다른 값으로 가면 표면이 새로 갈린다. `ui-standard.md` 6.6절에 기록했다. 2절 인벤토리에 잡히지 않았던 항목이다.

- 검증: `typecheck` · `vitest` **360 passed** · 빌드 2종 · 사내판 E2E **4 passed**(SDK 경로 포함)
- **실제 목업에 주입해 computed 값으로 확인**: 패널 글자 `#1a1a1a → rgb(15,23,42)`, 테두리 `#e2e2e2 → rgb(226,232,240)`, seg 활성 `#2f6feb → rgb(79,70,229)`, PID `#888 → rgb(100,116,139)`
- **전후 스크린샷 비교**: 팔레트만 바뀌고 **레이아웃·글자 크기는 변화 없음**(크기를 건드리지 않았다는 3.0절 제약의 실증)
- 번들: `sdk.js` 856,756B → 859,290B (**+2,534B**, 토큰 27종 + 헬퍼)
...
- **Codex P2 2차 1건도 반영 (내 1차 수정이 만든 새 문제였다).** `-2px`로 링을 안쪽에 그리자 **활성 세그먼트에서는 링이 자기 배경(`--c-btn-bg` `#4f46e5`) 위에 놓여 1.41:1로 묻혔다** — 포커스가 반드시 보여야 하는 바로 그 상태에서 안 보인다. 흰 링으로 분리했다(6.29:1). **`outline-offset`을 음수로 바꾸면 링이 "요소 자신의 배경" 위로 옮겨가므로 대비를 다시 재야 한다** — `.fab`에 흰 띠를 덧댄 것과 같은 사고다
- **⚠️ 규약 (사용자 지시, 2026-08-02): 추가 커밋을 푸시하면 반드시 `@codex review`를 단다.** Codex는 새 커밋에 자동 재리뷰하지 않는다 — 멘션이 유일한 재트리거다. 실제로 이번에 1차 수정을 푸시하고 멘션을 달았기에 2차 지적을 받았고, 그게 위의 대비 결함을 잡았다
  - **단, `docs/PROGRESS.md`만 바뀐 커밋에는 걸지 않는다** (사용자 지시, 2026-08-02). AGENTS.md 6절이 이 파일을 **"리뷰 대상이 아니라 다음 세션이 읽을 상태 기록"** 으로 규정하고 `main` 직접 커밋 예외까지 둔 이유가 그것이다. PR #94에서 이 원칙을 어겼더니 **Codex 지적 3건이 전부 PROGRESS 산문에 대한 것**이었고, 정작 이 PR의 실질 변경인 CSS 한 줄은 아무도 다시 보지 않았다. 고칠 때마다 새 문장이 생겨 다음 라운드 대상이 되는 구조라 **PR #89에서 8라운드를 돌았던 그 함정과 같다**
  - 실무 지침: **코드가 딸린 작업이면 PROGRESS는 병합 후 `main`에 직접 커밋**하는 쪽이 낫다(6절이 허용하는 두 방식 중 하나). 브랜치에 함께 넣으면 리뷰 대상이 된다
- **Codex는 `@codex review`를 달고 약 4분 뒤 도착했다.** 리뷰는 코멘트가 아니라 **`reviews` 배열 + 인라인 코멘트**로 온다 — `gh pr view --json comments`에는 **안 잡힌다.** 실제로 이 때문에 2차 지적이 도착한 줄 모르고 "리뷰 종료"를 선언할 뻔했다(사용자가 "피드백 다시 체크"로 잡아 줬다). **`--json reviews`와 `gh api repos/{owner}/{repo}/pulls/{n}/comments`를 함께 볼 것**
...
다음 할 일:
1. **PR #92 리뷰 대응.** 쌓아 둔 브랜치 4개를 사용자 요청으로 하나(`fix/ui-standard-remaining-87`)로 합쳐 올렸다. **리뷰 종료 조건을 PR 본문에 먼저 적어 뒀다** — 코드 파일 지적과 P1만 반영하고 문서 P2는 별건으로 돌린다(PR #89에서 8라운드를 돈 교훈)
2. 병합되면 `Closes #87`로 이슈가 닫힌다. **병합은 건별 사용자 동의가 필요하다**

막힌 지점: 없음 (Q-03 해소로 #87의 미결 결정이 모두 정리됐다. Q-07(히어로 미리보기 폭)만 "기본값으로 진행" 상태로 남아 있다)

### 2026-08-02 — T87-12 포커스 링 통일 (F-03 해소)

완료:
- **공통 규칙 한 블록씩**을 공개판 `globals.css`·사내판 `shell.ts`·SDK `styles.ts`에 넣었다. 형태는 4.6절대로 `outline: 2px solid` + `offset 2px`, `:focus-visible`만. 사내판은 **`shell.ts` 한 곳이면 콘솔·가이드·FAQ에 다 닿는다** — `console.ts`(931줄)·`pages.ts`에 따로 넣지 않았다
- **범위를 공개판까지 넓혔다.** 6.2절 표는 "사내판·SDK"였지만 실측해 보니 **공개판 버튼·링크도 브라우저 기본 링**이었다. 사내판만 고치면 두 배포가 새로 갈린다 — F-03을 고치려다 F-03을 만드는 셈이다
- **input 계열은 제외**했다(공개판·사내판이 이미 box-shadow 링 규약으로 일치). 다만 **SDK는 그 규약이 없어 input도 포함** — SDK 안에서 버튼만 링을 주면 그 안이 갈린다
- **SDK는 링 색을 `#2f6feb` 리터럴로 적었다.** Shadow DOM이라 `:root`가 닿지 않는다 — T87-13에서 `THEME_TOKENS`를 `:host`에 선언하며 토큰으로 바꾼다
- **`.fab`·`.panel-tab`·`.marker`에는 흰 띠를 덧댔다.** 이 셋만 사용자의 목업 페이지 위에 직접 떠서 링 바깥 색을 통제할 수 없고, 특히 `.fab`은 배경이 링과 같은 `#2f6feb`라 대비가 1:1이 된다. `.marker`가 흰 테두리로 같은 문제를 푸는 방식과 같다

**⚠️ 측정 방법이 결론을 바꿨다 — "링이 있는가"로 재면 안 된다.**
브라우저 기본 링도 링이라, 유무만 재면 **변경 전에도 "링 없는 요소 0건"으로 통과**한다.
실제 지표는 **형태의 종류 수**다. 공개판 4라우트 + 사내판 3라우트를 Tab으로 순회해 재니
**4종 → 2종**(통일 링 20개 + 입력 규약 2개)이었다. 앞으로 "일관성" 결함을 검증할 때는
**유무가 아니라 종류 수**를 센다.

**⚠️ `transition: all`이 포커스 링을 늦춘다 (이번에 발견해 함께 고침).**
사내판 `.c-btn`이 `transition: all 0.15s ease`였고 `all`은 **outline까지 전이 대상으로 잡는다.**
링이 0.15초에 걸쳐 나타났고 그 사이에는 밝은 배경에서 잘 안 보이는 기본 링(3px 흰색)이 보였다.
`Tab 직후 = 3px solid white`, `300ms 후 = 2px solid indigo`로 확인했다. 전이가 실제로 필요한
것은 hover의 배경·테두리·글자색뿐이라 그 셋만 명시했다. **공개판 `.c-btn`에는 transition이
아예 없어 이 문제가 없었다 — 사내판만의 문제였다. `transition: all`을 새로 쓰지 않는다.**

**측정 중 헛다리 하나:** 처음에 사내판만 규칙이 안 먹는 것처럼 보였는데, **서버 프로세스가
옛 dist를 물고 있었던 것**이다. CSS를 TS로 내보내는 사내판은 **빌드 후 서버를 반드시 재기동**해야
바뀐 CSS가 나온다. 공개판(`next start`)도 같다.

- 검증: `typecheck` · `vitest` **360 passed** · 빌드 2종 · 사내판 E2E **4 passed** · 공개판 E2E **7 passed**(1본은 #91의 기존 실패) · Tab 순회 **88개 요소** 링 없음 0건 · 대비 SC 1.4.11 전부 통과(라이트 4.27~4.47:1 · 다크 4.90~5.98:1 · 터미널 3.67~4.04:1 · SDK 4.38~4.57:1)
- **SDK는 실제 목업에 주입해 확인했다** — Shadow DOM이라 일반 Tab 순회로는 잡히지 않는다. 사내판 서버에 fixture zip을 올려 `{id}.localhost:4403`을 띄우고 Shadow root 안에서 직접 쟀다

**한 번 막힌 곳:** CSS가 TS 템플릿 리터럴 안이라 **주석에 백틱을 쓰면 리터럴이 끊긴다.**
`shell.ts` 주석에 `` `:focus` ``라고 적었다가 TS1005로 빌드가 깨졌다. 사내판 CSS를 고칠 때 주의.

다음 할 일:
1. **커밋만 하고 대기** (사용자 지시). 브랜치 `fix/focus-ring-unify-87`, base는 `chore/theme-tokens-single-source-87`
2. **T87-13이 #87 종결의 마지막 관문** — **Q-03(SDK 다크 모드) 결정이 선행**한다. SDK 팔레트·반경·다크 + `THEME_TOKENS` 적용 + T87-12가 넣은 `#2f6feb` 리터럴의 토큰화를 한 번에 간다

막힌 지점: Q-03 미결 — 이것만 정해지면 #87을 닫을 수 있다

### 2026-08-02 — T87-11 테마 토큰 단일 원천 (F-02 해소)

완료:
- **`packages/shared/src/theme.ts` 신설** — `THEME_TOKENS`(라이트·다크 27종씩)와 `themeTokenDeclarations(mode)` 헬퍼. **다크가 라이트의 키를 다 갖도록 타입으로 강제**한다(`Record<keyof typeof light, string>`) — 토큰을 추가하며 한쪽을 빠뜨리는 실수가 컴파일에서 막힌다
- **사내판 `shell.ts`에서 복제가 사라졌다** — 두 `:root` 블록의 토큰 54줄이 헬퍼 호출 2줄로 바뀌었다. **셀렉터·`color-scheme`·중괄호는 호출부에 남겼다** — SDK는 Shadow DOM이라 `:host`가 필요해서, 그릇까지 헬퍼가 정하면 SDK가 못 쓴다
- **공개판 `globals.css`는 파일로 유지**하고 파리티 테스트로 묶었다 (4.7절 판단 그대로 — CSS를 JS로 주입하면 FOUC·SSR 복잡도를 사는 대신 얻는 것이 없다)
- **`packages/shared/src/theme.parity.test.ts` 신설.** 정본이 있는 곳에 둔다 — 소비처마다 두면 새 소비처가 생길 때 빠진다. `globals.css` 값 대조 + **`shell.ts`에 토큰이 다시 하드코딩되지 않았는지**까지 본다(그 경로만 파리티를 우회할 수 있다)
- **`--c-label-w`·`--c-row-gap`은 넣지 않았다** — 테마가 아니라 레이아웃 값이고 라이트/다크 구분이 없다. 파리티 테스트가 명시적 예외로 다룬다

**⚠️ 테스트가 실제로 실패를 잡는지 확인했다 — 통과만으로는 근거가 되지 않는다.**
`globals.css`의 값 하나를 바꿨을 때 · 토큰 한 줄을 지웠을 때 · `shell.ts`에 토큰을 손으로
되돌려 넣었을 때, **세 경우 모두 해당 검사가 실패**하는 것을 실행으로 확인하고 원복했다.
파리티 테스트를 새로 만들 때는 이 확인까지 해야 한다 — 안 그러면 "늘 통과하는 테스트"가 된다.

**회귀 없음의 근거:** `SHELL_CSS`의 **런타임 값이 전후로 완전히 동일**하다. `git stash`로
변경 전후를 각각 빌드해 같은 방식으로 덤프하고 `diff`했다(차이 0). 처음에는 소스 텍스트와
런타임 값을 비교해 선행 개행 1개 차이가 나왔는데, **추출 방식이 달라 생긴 차이**였다 —
동일 방식으로 다시 재니 완전 일치였다. 생성 코드를 비교할 때는 양쪽을 같은 경로로 뽑을 것.

- 검증: `typecheck` 통과 · `vitest` **360 passed**(48파일, T87-10 대비 +5) · `npm run build` + `next build` 통과 · 사내판 E2E **4 passed**

다음 할 일:
1. **커밋만 하고 대기** (사용자 지시 — PR 없음). 브랜치는 `chore/theme-tokens-single-source-87`, base는 `fix/faint-token-merge-87`이다 (main에서 따면 `--c-faint`가 되살아난 상태에서 작업하게 된다)
2. T87-12(F-03 포커스 통일) — 선행 조건 없음
3. T87-13(SDK 패널) — **Q-03 결정이 선행**. 4.7절이 SDK를 남겨 뒀으므로 **팔레트·반경·다크와 `THEME_TOKENS` 적용을 한 번에** 간다

막힌 지점: Q-03(SDK 다크 모드) 미결 — #87 종결이 T87-13에 걸려 있다

### 2026-08-02 — T87-10 `--c-faint` 토큰 통합 (F-01 해소) + #90 등록

완료:
- **Q-04가 "토큰 통합"으로 결정됐다(사용자).** `--c-faint`를 값만 올리면 라이트에서 `--c-muted`와 **같은 값**(`#64748b`)이 돼 중복 토큰이 남는다. 없애는 쪽이 T87-11(단일 원천)도 가볍게 한다
- **토큰 삭제 + 사용처 4곳 이관.** `globals.css`·`shell.ts`의 라이트·다크 블록에서 선언을 지우고, `.c-project-meta`·`.c-extension-steps li::before`·`.g-faq-item summary::before`·`.g-path dt`를 `--c-muted`로 옮겼다. **공개판과 사내판을 같은 커밋에서 고쳤다** — F-02 때문에 한쪽만 고치면 조용히 갈라진다
- **문서 3.4절의 경고("색만 올리면 위계를 잃는다")가 실제로 걸린 곳은 4곳 중 1곳뿐이었다.** `.g-path dt`만 짝인 `dd`(`--c-text-3`)와 색차가 좁아져 `font-weight: 500`으로 위계를 옮겼다(4.2절 규칙 2 그대로). 나머지 셋은 크기·굵기·서체·절대위치가 이미 구별을 만들고 있어 손대지 않았다. **크기·여백은 전혀 건드리지 않았다** — 3.0절의 확립된 사실과 4.2절 규칙 1에 걸린다
- 검증: `typecheck` 통과 · `vitest` **355 passed**(47파일) · `npm run build` + `next build` 통과 · 사내판 E2E **4 passed** · `git diff --check` 통과
- **대비 계산**: 변경 후 조합 전부 AA 통과. 가장 빡빡한 것이 라이트 `#f8fafc` 배경 위 **4.55:1**로 4.5:1을 0.05만 넘긴다 — 배경이나 `--c-muted`를 건드리면 즉시 미달로 돌아간다. 6개 조합 전부는 `ui-standard.md` 6.3절 표에 있다
- **실렌더 확인**(7.1절 절차): 공개판 `next start` + 사내판 `packages/server`를 각각 띄워 Playwright로 **네 사용처의 computed color를 실측**했다. 라이트 `rgb(100,116,139)` / 다크 `rgb(148,163,184)`로 `--c-muted`와 일치하고, `getPropertyValue("--c-faint")`가 **양쪽 배포·양쪽 테마에서 빈 문자열**이다(토큰 잔존 0건). `.g-path dt`의 `font-weight: 500`도 렌더에서 확인. 가로 오버플로는 390·768·1280·2400px × 라이트/다크에서 **0건**
- **#90 등록** — 사용자 지적으로 README 경로 D 스크린샷 1·3번째(콘솔 화면)의 노후화를 이슈로 남겼다. 헤더의 `DOCS`·`EN` 메뉴가 지금 없고, 확장 안내가 `packages/extension/dist` 직접 로드 시절 문구다

**⚠️ 공개판 E2E 1본이 실패하지만 이 작업과 무관하다 — `main`에서도 똑같이 실패한다 (→ #91 등록):**
`open-service-dod.spec.ts:879` (`경로 D 콘솔 — 생성·연결 코드·토큰 재발급 (#42)`)의
`getByRole("tab", { name: "ZIP 업로드" }).click()`이 180초 테스트 타임아웃에 걸린다.
**`git stash`로 변경을 걷어내고 `origin/main` 상태에서 같은 테스트를 돌려 동일 지점·동일 증상을 확인했다.**
실패 시점 스냅샷에는 그 탭이 보이는 상태로 존재한다(로그인도 성공). 나머지 7본은 통과한다.
이 환경(로컬 macOS + 실 Supabase) 특유인지 실제 회귀인지는 **미확인** — PR #88·#89의 CI에서는
이 스펙이 통과했으므로 별건으로 봐야 한다. **T87-10의 병합 판단에 이 실패를 근거로 삼지 말 것.**

다음 할 일:
1. **커밋·PR·병합은 사용자 지시 대기.** 브랜치 `fix/faint-token-merge-87`에 변경이 **미커밋 상태**로 있다 (AGENTS.md 6절 — 커밋/푸시는 요청 시에만)
2. 그 뒤 T87-11(F-02 토큰 단일 원천 — `shared`에 `THEME_TOKENS` + 파리티 vitest) → T87-12(F-03 포커스)
3. **T87-11 주의:** 지금 `--c-faint`가 빠져 두 파일의 토큰 수가 라이트 29/27·다크 27/27이다. 파리티 테스트는 레이아웃 토큰 2종(`--c-label-w`·`--c-row-gap`, 사내판은 `console.ts`에 별도 선언)을 예외로 다뤄야 한다

막힌 지점:
- Q-03(SDK 다크 모드) 미결 — T87-13의 선행 조건이다. **#87 종결이 T87-13에 걸려 있으므로 이 결정이 종결 경로 위에 있다**
- #91 (공개판 E2E 1본, main에서도 실패, 원인 미확인)

**사용자 결정 (2026-08-02):**
- **#87은 T87-11~13까지 끝내고 닫는다.** T87-14(산출물)는 #86 종속이라 이 이슈에서 하지 않는다 — 문서 6.2절과 같다
- T87-10은 **커밋만 하고 PR은 열지 않는다.** 병합 시점·묶음은 이후 지시를 따른다

### 2026-08-02 — #87 UI 규범 문서 + 로그인·히어로 채택 (PR #89 **병합 완료** `56b7735`)

완료:
- **브랜치는 `feat/ui-standard-87` 하나, PR도 #89 하나다** (사용자 요청으로 통합). 처음에는 성격별 3브랜치로 나눴다가 합쳤고, **커밋 단위로는 성격이 그대로 나뉘어 있다** — 리뷰는 커밋별로 보면 된다
- **`docs/ui-standard.md` 신설** — 네 표면의 시각 스타일 인벤토리·공통 기준. `docs/README.md` 문서 표와 **`AGENTS.md` 1절 필독 목록(8번)**에 등록
- **로그인** (`fix:`) — ① `<h1>`이 `.c-section-title` 재사용으로 13px이던 것을 `.c-login-title` **22px**로 분리 ② Google/매직링크가 둘 다 채움 강조색이라 주 동작이 없던 것을 `.c-btn-social` 테두리형으로 분리 ③ **인라인 `style` 4곳 제거** — 콘솔 2단 폼 그리드를 이 화면에서만 되돌리던 땜질을 `.c-login-card .c-row` CSS로 이관 ④ 서비스 설명 1줄 추가
- **랜딩 히어로** (`feat:`) — 좌우 2단 → **중앙 정렬 1단**. 제목 상한 62→58px, 미리보기 폭 860px 제한, 기울임 제거. **CSS만, 마크업 무변경**
- 검증: `npm run typecheck` 통과, `vitest` **342 passed / 13 skipped**(Supabase 연동분은 `.env.local` 부재로 설계대로 스킵), `npm run build` + `next build` 통과. 렌더는 Playwright로 확인 — 390/1440px × 라이트/다크에서 **가로 오버플로 0건**(`scrollWidth - clientWidth` 계산)

**⚠️ 다음 세션이 반드시 알아야 할 것 — A/B 5회차 중 3건이 기각됐다:**

| 회차 | 가설 | 판정 |
|---|---|---|
| 1 | 콘솔 본문 14px·제목 20px로 키운다 | **기각** — "원래 화면이 더 낫다" |
| 2 | 가이드·FAQ 본문 15px·줄 45자 | **기각** (로그인만 채택) |
| 3 | 히어로 미리보기의 빈 상자를 내용으로 채운다 | **기각** — "안 채우는 게 낫다" |
| 4 | 랜딩 자잘한 불일치 정돈(크기 4종→2종 등) | **철회** — 렌더 결과 보이는 차이 없음 |
| 5 | 히어로 중앙 정렬 1단 | **채택** |

**확립된 사실: 본문 글자 크기를 키우는 방향은 이 제품에서 채택되지 않는다.** 현재 밀도가 사용자 선호다. 채택된 것(로그인·중앙 정렬)은 전부 **구조적 결함**을 고친 경우다. 판단 기준 = "구조적 결함인가, 밀도 취향인가" — 전자만 손댄다. 상세는 `docs/ui-standard.md` 3.0절.

**방법론 교훈:** 인터랙티브 A/B 페이지(`*-ab.html`)는 창이 좁으면 패널이 세로로 쌓이고 iframe이 2600px라 **나란히 비교가 불가능했다.** 사용자가 "차이를 모르겠다"고 한 주원인이 디자인이 아니라 이 비교 도구였다. 앞으로 시각 검증은 **Playwright로 실제 앱을 띄워 찍은 정지 이미지**로 한다 — 절차는 `ui-standard.md` 7.1절.

봇 리뷰 11건 처리 (9건 반영·2건 기각). 반영분 중 **실제 회귀 1건**이 있었다:
- **`text-align` 상속** — `.l-hero`에 가운데 정렬을 걸어 미리보기 안 어노테이션 설명까지 끌려갔다. `.l-hero-copy`로 좁혀 해결. **가로 오버플로 0·테스트 전부 통과였는데도 새어 나갔다** — 상속 속성을 부모에 걸면 수치 검증으로 안 잡힌다는 교훈을 `ui-standard.md` 7.1절에 규칙으로 넣었다
- **새 크기 값을 만들었다가 되돌림** — 로그인·히어로에 24px·13.5px·56px를 새로 넣어 `globals.css`의 크기 종류가 16→18, gap이 17→18로 늘었다. **그게 바로 문서 4.2절이 금지한 행위**라 기존 값(22px·13px·54px)으로 재사용하도록 고쳤다. 지금 인벤토리 수치는 `main`과 같다
- 기각 2건: 날짜(KST 기준이 맞다 — PR #33의 UTC 선례), PROGRESS의 main 직접 커밋(아래 막힌 지점)

다음 할 일:
1. **PR #89 main 병합** (T87-9). 병합은 건별 명시적 동의 필요
2. 그 뒤 T87-10(F-01 대비) → T87-11(F-02 토큰 단일 원천) → T87-12(F-03 포커스) 순
3. **T87-10 주의:** `--c-faint`를 값만 올리면 그 화면이 위계를 잃는다(문서 3.2절). 크기·여백 조정과 함께 가야 한다

**병합 후 리뷰 루프에서 배운 것 (다음에 같은 함정을 피하려면):**
봇 리뷰 22건 중 **코드 파일 지적은 2건뿐**이었고(둘 다 진짜 회귀 — `text-align` 상속, 960px 폭 튐), 나머지 20건은 이번에 새로 추가한 `ui-standard.md`(500줄)의 **산문 다듬기**였다. 새 문서는 diff 전체가 리뷰 대상이라 봇이 푸시마다 통째로 다시 읽고, 산문에는 늘 더 정확히 쓸 문장이 남아 있어 **P2가 무한히 나온다.** 게다가 수정 자체가 새 문장을 만들어 다음 라운드의 대상이 된다. 8라운드를 돌고서야 멈췄다.
→ **큰 새 문서가 든 PR은 종료 조건을 먼저 정한다.** 예: "코드 파일 지적과 P1만 반영, 문서 P2는 별건." 봇 P2를 전부 필수로 취급하면 병합이 무한히 밀린다.

막힌 지점:
- ~~**`docs/PROGRESS.md`를 `main`에 직접 커밋하지 못했다**~~ — **해소.** 병합 후 `main` 워크트리(`C:/Users/surro/Toy/draftify-html`)를 `git pull --ff-only`로 따라잡아 이제 그곳에서 직접 커밋한다. **다음 세션도 PROGRESS는 그 워크트리에서 커밋할 것** — 이 클론(`orca/workspaces/.../ui`)은 `main`을 체크아웃할 수 없다
- (이전 기록) **`main`이 다른 워크트리에 잡혀 있던 문제** (AGENTS.md 6절 예외). 이 클론에서 `main`이 다른 워크트리(`C:/Users/surro/Toy/draftify-html`)에 체크아웃돼 있어 `git checkout main`이 거부된다. 그래서 이번에는 `feat/ui-standard-87` 브랜치(PR #89)에 함께 커밋했다 — **그 PR이 병합돼야 `main`에 반영된다.** 다음 세션은 워크트리 상황을 먼저 확인할 것
- Q-03(SDK 다크 모드)·Q-07(히어로 미리보기 폭 860px) 미결 — 기본값으로 진행 중
