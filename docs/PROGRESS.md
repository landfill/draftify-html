...
**▶ 다음 세션 시작점 (2026-08-01 갱신) — #71 1단계(함수 리전 `icn1`) **완료·프로덕션 반영**(PR #80, merge `1ec760a`). 콘솔 홈 목록 표시가 **7.1초 → 1.15초**. **#71은 열려 있다** — 남은 후보는 코드 측 3건(RSC 프리페치·N+1·`getUser` 중복)이고, 착수 여부는 사용자 판단 대기.**
...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - **#71** 공개판 로딩 지연 — **1단계(함수 리전) 완료·프로덕션 반영(PR #80). 이슈는 열려 있다.** 남은 것은 코드 측 3건: ① **RSC 프리페치**(가장 큼 — 목록 표시 1,151ms 중 656ms가 클라이언트 워터폴) ② N+1(프로젝트당 export 요약 조회 — 현재 프로젝트 1개라 영향 미미, 수가 늘면 부상) ③ `getUser()` 중복 + 목록에 불필요한 `spec` JSONB 전체 전송. **이슈가 후보로 든 "Supabase 조회 튜닝"은 실측이 기각했다** — 상세는 맨 위 세션 로그
  ...
  - **#12 · #13** `[실수요 대기]` 유지
  - 종결: **#64·#70(PR #79)**, **#77(PR #78)**, **#65(PR #76)**, **#68(PR #75)**, **#63(PR #72)**, #43(PR #67), #47(PR #58), #50·#51(PR #53), #34·#36·#42(이전 세션)
...
### 2026-08-01 — #71 1단계: 함수 리전 `icn1` 고정 (PR #80 병합)

- 완료: **PR #80 병합 (merge `1ec760a`), 프로덕션 반영·재측정까지 끝.** **제품 코드 0줄** — `apps/web/vercel.json` 신설(`regions: ["icn1"]`) + 근거 문서. **이슈 #71은 열어 뒀다**(코드 측 개선이 남았다).
...
- **개선 결과 (프로덕션, 동일 브라우저·동일 방법·warm 중앙값)**:
  ...
  | **콘솔 홈 목록 표시 완료** | **7,115ms**(콜드) | **1,151ms** |
...
- 다음 할 일: **#71 코드 측 3건은 착수 여부가 사용자 판단 대기.** 실익 순서는 ① **RSC 프리페치** — 목록 표시 1,151ms 중 **656ms가 클라이언트 워터폴**(HTML 495ms 완료 → 48ms 뒤 API 시작 → 608ms)이라 이게 가장 크다 ② `getUser()` 중복·`spec` 전체 전송 ③ N+1(프로젝트 1개인 지금은 실익 거의 없음 — 수가 늘면 부상). **왕복 단가가 ~170ms → ~16ms로 떨어져 ②③의 값어치는 리전 이동 전보다 크게 줄었다.**
- 막힌 지점: 없음.
