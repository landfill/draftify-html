...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - **#87** 전체 UI 스타일 — **진행 중.** 문서(`docs/ui-standard.md`) + 채택 2건(로그인·히어로 중앙 정렬) 커밋 완료, PR 대기. 나머지 표면은 현재 유지 결정. **산출물 코드는 범위 밖 → #86**
...
## 전체 UI 스타일 WBS 체크리스트 (이슈 #87, 2026-08-02 착수 — **문서 + 채택 2건 커밋, PR 대기**)
...
- [ ] T87-14 산출물 — **#86 완료 + AGENTS.md 4절 절차 선행.** 이 이슈에서 하지 않는다
