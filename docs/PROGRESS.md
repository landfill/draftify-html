...
다음 할 일:
- **사용자 결정 2건 대기** — ① 모바일 폭 값(**390px 권장** — #93 실측·`ui-standard.md` 7절이 이미 390을 좁은 폭 표준으로 쓴다. PC는 `captureWidth` 그대로 = 현행) ② 적용 단위(**`Scene` 필드 + 직전 화면 값 프리필 권장** — 프로젝트 기본값 필드를 안 두고도 반복 입력이 사라진다. `pageSectionLabel`이 T38-4에서 쓴 패턴)
- 정해지면 `s1-kickoff-spec.md` 11절 **21차 개정**부터 쓴다 (AGENTS.md 4절 — 코드보다 스펙이 먼저). 구현은 파일 4개: `shared`(`Scene.targetDevice?`) · `sdk/ui/App.tsx`(선택 UI·프리필) · `viewer/src/main.ts`(`baseWidth`) · `export.ts`(스테이지 CSS)
- **호환**: `targetDevice` 없으면 `desktop`으로 읽어 현행 동작 유지. `captureWidth` 폴백 계약도 그대로

막힌 지점: **위 결정 2건.** (앞서 적었던 "결정 4건 / 비용 차 한 자릿수 배"는 증상 확인으로 해소됐다 — 2·3단계가 빠져 선택지가 줄었다.)
