...
**▶ 다음 세션 시작점 (2026-08-02 갱신) — **#87에서 할 일이 전부 끝났다** (T87-10·11·12·13). 작업하며 브랜치를 4개로 쌓았지만 **사용자 요청으로 하나(`fix/ui-standard-remaining-87`, 9커밋)로 합쳐 올린다** — PR #89 때와 같은 방식이고, 커밋 단위로는 성격이 그대로 나뉘어 있어 커밋별로 리뷰하면 된다. **다음 할 일은 그 PR의 리뷰 대응이고, 병합되면 #87이 닫힌다.** T87-14(산출물)는 #86 종속이라 이 이슈에서 하지 않는다. UI를 건드리기 전에 `docs/ui-standard.md` 3.0절(기각 이력)을 반드시 읽는다.**
...
다음 할 일:
1. **커밋만 하고 대기** (사용자 지시). 브랜치 `feat/sdk-palette-87`, base는 `fix/focus-ring-unify-87`
2. **#87을 닫을 수 있다** — 사용자 결정대로 T87-11~13이 끝났다. T87-14(산출물)는 #86 종속이라 이 이슈에서 하지 않기로 문서 0절에 이미 정해져 있다
3. 쌓인 브랜치 4개의 처리(PR·병합 순서)를 정해야 한다 — 순서대로 쌓여 있어 아래에서부터 병합해야 한다
