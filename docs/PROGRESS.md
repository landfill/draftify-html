...
**▶ 다음 세션 시작점 (2026-08-02 갱신) — T87-10·T87-11 완료. 둘 다 **로컬 브랜치에 커밋만 돼 있고 푸시·PR은 없다**(사용자 지시): `fix/faint-token-merge-87` → 그 위에 `chore/theme-tokens-single-source-87`. 다음 착수는 T87-12(F-03 포커스 통일, 선행 조건 없음). UI를 건드리기 전에 `docs/ui-standard.md` 3.0절(기각 이력)을 반드시 읽는다.**
...
- [ ] T87-12 F-03 포커스 통일 — 사내판 콘솔·SDK에 `:focus-visible` 링
