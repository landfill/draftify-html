...
**▶ 다음 세션 시작점 (2026-08-02 갱신) — T87-10·11·12 완료. 셋 다 **로컬 브랜치에 커밋만 돼 있고 푸시·PR은 없다**(사용자 지시). 브랜치가 순서대로 쌓여 있다: `fix/faint-token-merge-87` → `chore/theme-tokens-single-source-87` → `fix/focus-ring-unify-87`. **남은 것은 T87-13 하나이고, 그것이 #87 종결 조건이다 — 착수하려면 Q-03(SDK 다크 모드) 결정이 먼저 필요하다.** UI를 건드리기 전에 `docs/ui-standard.md` 3.0절(기각 이력)을 반드시 읽는다.**
...
- [ ] T87-13 SDK 패널 — 팔레트·반경·다크. **크기는 키우지 않는다**(3.0절). Q-03 선행
