...
**▶ 다음 세션 시작점 (2026-08-02 갱신) — #87 UI 규범 문서 + 채택된 코드 2건이 **PR #89 하나**(`feat/ui-standard-87`)에 올라가 있다. CI 6체크 green·리뷰 반영 완료, **병합 대기**. UI를 건드리기 전에 `docs/ui-standard.md` 3.0절(기각 이력)을 반드시 읽는다.**

**디자인 이슈 진행 상태 (2026-08-02):**

- **#87 전체 UI 스타일** — **PR #89 (`feat/ui-standard-87`) 병합 대기.** 브랜치는 **하나다**(사용자 요청으로 통합). 커밋만 성격별로 나눠 뒀다: `docs:`(규범 문서) → `fix:`(로그인) → `feat:`(히어로 중앙 정렬) → `docs:`(이 PROGRESS) → 리뷰 반영 3커밋. **⚠️ A/B 검증에서 "본문 글자를 키우는" 방향이 세 번 기각됐다** — 콘솔·가이드·FAQ·랜딩 프리뷰는 현재 유지다. 채택된 것은 구조적 결함을 고친 경우뿐. 상세는 `docs/ui-standard.md` 3.0절
...
### 2026-08-02 — #87 UI 규범 문서 + 로그인·히어로 채택 (PR #89, 병합 대기)
...
막힌 지점:
- **`docs/PROGRESS.md`를 `main`에 직접 커밋하지 못했다** (AGENTS.md 6절 예외). 이 클론에서 `main`이 다른 워크트리(`C:/Users/surro/Toy/draftify-html`)에 체크아웃돼 있어 `git checkout main`이 거부된다. 그래서 이번에는 `feat/ui-standard-87` 브랜치(PR #89)에 함께 커밋했다 — **그 PR이 병합돼야 `main`에 반영된다.** 다음 세션은 워크트리 상황을 먼저 확인할 것
