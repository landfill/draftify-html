...
**▶ 다음 세션 시작점 (2026-08-02 갱신) — #87 1차 완료·main 병합 (PR #89, merge `56b7735`). 프로덕션 반영됨. 다음 착수는 T87-10(F-01 `--c-faint` 대비). UI를 건드리기 전에 `docs/ui-standard.md` 3.0절(기각 이력)을 반드시 읽는다.**

**디자인 이슈 진행 상태 (2026-08-02):**

- **#87 전체 UI 스타일** — **PR #89 main 병합 완료 (merge `56b7735`, 2026-08-02). 이슈는 계속 열려 있다** — 남은 것은 T87-10~14. 브랜치는 **하나였다**(사용자 요청으로 통합). 커밋만 성격별로 나눠 뒀다: `docs:`(규범 문서) → `fix:`(로그인) → `feat:`(히어로 중앙 정렬) → `docs:`(이 PROGRESS) → 리뷰 반영 3커밋. **⚠️ A/B 검증에서 "본문 글자를 키우는" 방향이 세 번 기각됐다** — 콘솔·가이드·FAQ·랜딩 프리뷰는 현재 유지다. 채택된 것은 구조적 결함을 고친 경우뿐. 상세는 `docs/ui-standard.md` 3.0절
...
- [ ] T87-10 F-01 `--c-faint` 대비 수정 — `globals.css`·`console.ts` 동시(F-02 때문에). **3.2절에 따라 색만 올리면 그 화면이 위계를 잃는다** — 크기·여백과 함께 가야 한다
