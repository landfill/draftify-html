...
**▶ 다음 세션 시작점 (2026-07-31 갱신) — #63·#68·#65·#77 프로덕션 반영 완료. #64(문서 현행화)·#70(`§` 표기)은 브랜치 `docs/index-conventions-and-section-notation-64-70`에서 작업 완료 → **PR 리뷰·병합 대기**(문서만 변경, 코드 0줄). 그 다음 후보는 #71(공개판 로딩 지연).**
...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - **#70** 문서의 `§` 기호를 '절'로 풀어쓰기 — **작업 완료, PR 대기** (856곳 치환 + AGENTS.md 표기 규약 신설)
  - **#64** 문서 인덱스·작업 규약 현행화 — **작업 완료, PR 대기** (guide 7종 목록화·읽는 순서에 user-guide 추가·docs/README 세 경로 반영·`apps/web/README.md` 신설)
...
### 2026-07-31 — #64 문서 인덱스·규약 현행화 + #70 `§` 표기 정리 (PR 대기)

- 완료: 브랜치 `docs/index-conventions-and-section-notation-64-70`, 커밋 2개(이슈별 분리). **문서만 변경 — 코드 0줄.**
- **#64 (4개 완료 조건 전부)**:
