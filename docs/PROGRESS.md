...
**▶ 다음 세션 시작점 (2026-08-02 갱신) — **#87 종결, 후속까지 병합 완료. 진행 중인 작업이 없다.** PR #92(merge `aa839cb`)로 T87-10~13이, PR #94(merge `255e9cb`)로 그 리뷰에서 나온 마지막 접근성 수정이 프로덕션에 나갔다. **다음 세션은 아래 "열린 이슈"에서 골라 시작하면 된다** — 우선순위는 `#86`(산출물 UI. 이것이 끝나야 산출물 시각 통일을 착수할 수 있다) → `#93`(사내판 390px 넘침) → `#90`(README 스크린샷). UI를 건드리기 전에 `docs/ui-standard.md` **3.0절(기각 이력)** 과 **6.5절(포커스 링 함정)** 을 반드시 읽는다.**
...
- **#93 사내판 콘솔 390px 가로 넘침** — 신규 등록 (2026-08-02). `.c-project-actions`의 버튼 5개가 줄바꿈되지 않아 67px 넘친다. `ui-standard.md` F-05가 "미검증"으로 남겨 둔 항목을 실측한 결과다. 공개판은 0이라 **두 배포가 이 지점에서 갈려 있다**
...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - **#93** 사내판 콘솔이 390px에서 67px 넘친다 — `.c-project-actions`의 버튼 5종이 줄바꿈되지 않는다. `ui-standard.md` F-05가 "미검증"으로 남겨 둔 항목을 실측한 결과이고, **공개판은 0이라 두 배포가 이 지점에서 갈려 있다**
  ...
  - **#12 · #13** `[실수요 대기]` 유지
  - 종결: **#85(PR #88)**, **#81(PR #84)**, **#82(PR #83)**, **#71(PR #80)**, #64·#70(PR #79), #77(PR #78), #65(PR #76), #68(PR #75), #63(PR #72), #43(PR #67), #47(PR #58), #50·#51(PR #53), #34·#36·#42(이전 세션)
