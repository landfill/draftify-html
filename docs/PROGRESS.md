...
**▶ 다음 세션 시작점 (2026-08-02 갱신) — #87 UI 규범 문서 + 채택된 코드 2건을 브랜치 3개에 커밋 완료, PR·병합 대기. 코드 적용 전에 `docs/ui-standard.md` 3.0절(기각 이력)을 반드시 읽는다.**

**디자인 이슈 진행 상태 (2026-08-02):**

- **#87 전체 UI 스타일** — **문서 + 채택 2건 커밋 완료, PR 대기.** 브랜치 3개: `docs/ui-standard-87`(문서·이 PROGRESS), `fix/login-screen-87`(로그인), `feat/landing-hero-center-87`(히어로 중앙 정렬). **⚠️ A/B 검증에서 "본문 글자를 키우는" 방향이 두 번 기각됐다** — 콘솔·가이드·FAQ는 현재 유지다. 채택된 것은 구조적 결함을 고친 경우뿐(로그인). 상세는 `docs/ui-standard.md` 3.0절
...
- [x] T87-5 문서 커밋 — `docs/ui-standard-87` (`2a293f4`)
- [x] T87-6 로그인 화면 — `fix/login-screen-87` (`72e91b1`). 13px `<h1>`·버튼 위계·인라인 style 제거·서비스 설명
- [x] T87-7 랜딩 히어로 중앙 정렬 — `feat/landing-hero-center-87` (`338d9d6`). CSS만, 마크업 무변경
- [ ] T87-8 PR 3건 생성·리뷰·main 병합 — **병합은 건별 명시적 동의 필요**
- [ ] T87-9 F-01 `--c-faint` 대비 수정 — `globals.css`·`console.ts` 동시(F-02 때문에). **3.2절에 따라 색만 올리면 그 화면이 위계를 잃는다** — 크기·여백과 함께 가야 한다
- [ ] T87-10 F-02 토큰 단일 원천 — `shared`에 `THEME_TOKENS` + `globals.css` 파리티 vitest
- [ ] T87-11 F-03 포커스 통일 — 사내판 콘솔·SDK에 `:focus-visible` 링
- [ ] T87-12 SDK 패널 — 팔레트·반경·다크. **크기는 키우지 않는다**(3.0절). Q-03 선행
- [ ] T87-13 산출물 — **#86 완료 + AGENTS.md 4절 절차 선행.** 이 이슈에서 하지 않는다

## 세션 로그 (최신이 위)

### 2026-08-02 — #87 UI 규범 문서 + 로그인·히어로 채택 (브랜치 3개 커밋, PR 대기)

완료:
- **`docs/ui-standard.md` 신설** — 네 표면의 시각 스타일 인벤토리·공통 기준. `docs/README.md` 문서 표에 등록. 커밋 `2a293f4` (`docs/ui-standard-87`)
- **`fix/login-screen-87` (`72e91b1`)** — 로그인 ① `<h1>`이 `.c-section-title` 재사용으로 13px이던 것을 `.c-login-title` 24px로 분리 ② Google/매직링크가 둘 다 채움 강조색이라 주 동작이 없던 것을 `.c-btn-social` 테두리형으로 분리 ③ **인라인 `style` 4곳 제거** — 콘솔 2단 폼 그리드를 이 화면에서만 되돌리던 땜질을 `.c-login-card .c-row` CSS로 이관 ④ 서비스 설명 1줄 추가
- **`feat/landing-hero-center-87` (`338d9d6`)** — 랜딩 히어로를 좌우 2단 → **중앙 정렬 1단**. 제목 상한 62→58px, 미리보기 폭 860px 제한, 기울임 제거. **CSS만, 마크업 무변경**
...
**방법론 교훈:** 인터랙티브 A/B 페이지(`*-ab.html`)는 창이 좁으면 패널이 세로로 쌓이고 iframe이 2600px라 **나란히 비교가 불가능했다.** 사용자가 "차이를 모르겠다"고 한 주원인이 디자인이 아니라 이 비교 도구였다. 앞으로 시각 검증은 **Playwright로 렌더한 정지 이미지를 나란히 합성해** 보여 준다 — 절차는 `ui-standard.md` 7.1절.

다음 할 일:
1. **PR 3건 생성 → 리뷰 → main 병합** (T87-8). 병합은 건별 명시적 동의 필요
2. 그 뒤 T87-9(F-01 대비) → T87-10(F-02 토큰 단일 원천) → T87-11(F-03 포커스) 순
3. **T87-9 주의:** `--c-faint`를 값만 올리면 그 화면이 위계를 잃는다(문서 3.2절). 크기·여백 조정과 함께 가야 한다

막힌 지점:
- **`docs/PROGRESS.md`를 `main`에 직접 커밋하지 못했다** (AGENTS.md 6절 예외). 이 클론에서 `main`이 다른 워크트리(`C:/Users/surro/Toy/draftify-html`)에 체크아웃돼 있어 `git checkout main`이 거부된다. 그래서 이번에는 `docs/ui-standard-87` 브랜치에 함께 커밋했다 — **그 PR이 병합돼야 `main`에 반영된다.** 다음 세션은 워크트리 상황을 먼저 확인할 것
