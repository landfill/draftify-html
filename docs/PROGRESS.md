...
**▶ 다음 세션 시작점 (2026-08-01 갱신) — #85 공개판 메인 화면 구현 완료·병합 대기. 비로그인 `/`은 서비스 랜딩, 로그인 `/`은 프로젝트 콘솔이며 재방문 콘솔은 목록을 먼저 보여준다.**

**디자인 이슈 진행 상태 (2026-08-01):**

- **#85 공개판 메인 화면** — **`feat/public-home-85`에서 구현 완료·병합 대기.** 새 경로를 만들지 않고 `/`을 세션에 따라 분기한다. 비로그인에는 서비스 소개·사용 흐름·`/guide`·`/sample` 진입점을, 로그인에는 기존 콘솔을 보여준다. 프로젝트가 있으면 목록 우선·생성 폼 접힘, 없으면 생성 폼 우선·펼침이다
...
- **열린 이슈** — 처리 순서는 위 "다음 할 일":
  ...
  - **#85** 공개판 메인 화면 디자인 — **구현 완료·병합 대기** (`feat/public-home-85`)
...
- [ ] T85-6 PR·Preview·main 병합 — 커밋·푸시·PR은 사용자 요청 대기, main 병합은 건별 명시적 동의 필요
...
- 다음 할 일: 사용자가 요청하면 커밋·푸시·PR을 만들고 Preview에서 실제 Supabase 세션으로 빈 상태/재방문 상태를 확인한다. 리뷰 반영 후 main 병합은 별도 명시적 동의를 받는다.
- 막힌 지점: 코드 구현은 없음. 이 작업 환경에는 `apps/web/.env.local`이 없어 실제 Supabase 연동 E2E는 실행하지 못했고 Playwright 목록 컴파일까지만 확인했다. PR/병합은 사용자 요청·동의 대기.
