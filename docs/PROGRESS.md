...
**▶ 다음 세션 시작점 (2026-08-02 갱신) — **#87에서 할 일이 전부 끝났다** (T87-10·11·12·13). 넷 다 **로컬 브랜치에 커밋만 돼 있고 푸시·PR은 없다**(사용자 지시). 브랜치가 순서대로 쌓여 있어 **아래에서부터 병합해야 한다**: `fix/faint-token-merge-87` → `chore/theme-tokens-single-source-87` → `fix/focus-ring-unify-87` → `feat/sdk-palette-87`. **다음 할 일은 이 4개의 PR·병합 방식을 정하는 것이고, 그 뒤 #87을 닫는다.** T87-14(산출물)는 #86 종속이라 이 이슈에서 하지 않는다. UI를 건드리기 전에 `docs/ui-standard.md` 3.0절(기각 이력)을 반드시 읽는다.**
