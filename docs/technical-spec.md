...
>
> **스키마 정본 (2026-08-06, 이슈 #57)**: 공개판 Postgres 스키마의 현재 모습은 **`apps/web/supabase/schema.sql` 한 파일**이 답이다. `migrations/`는 적용 수단으로 남긴다. 변경 절차(정본 먼저 → 마이그레이션 1본 추가 → 적용 순서 판단 → 사용자 동의 후 적용)는 **AGENTS.md 6절 "스키마를 바꿀 때"** 가 규약이다. **Supabase는 하나뿐이라 Preview·Production이 같은 DB를 본다** — 적용하는 순간 프로덕션에 걸린다.
