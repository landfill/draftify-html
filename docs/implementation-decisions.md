...
> 킥오프 스펙의 기존 결정을 변경하는 항목(ID-03, ID-04)은 guide/s1-kickoff-spec.md 11절에 기록했다.
...
- **버린 대안**: 킥오프 스펙 8절의 `localhost:4000/api` + `*.localhost` CORS 허용 — 동작하지만 설정 한 겹이 더 있고 오리진 하드코딩(ID-01 위반)을 유발한다. 11절에 변경 기록.
...
- **이유**: 킥오프 스펙의 "뷰포트 대비 비율"은 부착 시점 스크롤 위치에 따라 같은 요소가 다른 값을 갖는다 — fallback으로서 성립하지 않음. 스키마는 동일, 의미만 변경. 11절에 기록.
...
- **결정**: **패널에서 선택된 장면이 곧 현재 장면.** 라우트와 자동 연동하지 않는다.
  ...
  - 미리보기 모드에서 라우트를 이동해도 현재 장면은 유지된다. 라우트 변경 배너(detailed-spec 3.6절)만 뜬다
  - 장면 등록 시 그 장면이 자동으로 현재 장면이 된다 (기존 사양 그대로)
- **라우트 제안 경계 (킥오프 11절 16차)**: `pathname+search+hash` 변경을
  `pushState`·`replaceState`·`popstate`·`hashchange`로 감지한다. 패널/모드와 무관하게
  감지하고 닫힌 패널에서는 제안을 보존한다. 최초 route는 이미 본 것으로 간주하며 이후
  route별 세션 1회만 "새 화면으로 등록할까요?"를 제안한다 — 1회의 소진 기준은 배너가
  실제로 표시된 route(11절 17차). route로 현재 장면을 자동 전환하거나 기존 장면과
  매칭하지 않는다.
...
- 재캡처 성공 시 이전 스냅샷 asset **즉시 삭제**, 장면 삭제 시 소속 asset 즉시 삭제
...
- 캡처 시 `<script>` 0개 검증(POL-M06)과 **이중 방어**: 검증을 뚫은 인라인 이벤트 핸들러 등도 sandbox가 막는다.
