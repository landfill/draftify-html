...
> **[수정 필요]** (현 배포 = 사내 서비스. 공개 URL 멀티테넌트 전환은 **open-service 트랙** — 아래 §6·§7.)
> <!-- 수정 필요 (2026-07-30 확인)
>      1) 사실관계 낡음: 공개판이 https://draftify-html.vercel.app 으로 이미 배포되어 main이 프로덕션(루트 README.md 254행).
>         "현 배포 = 사내 서비스"는 현재 상태와 다르고, open-service 트랙은 종료되어 브랜치도 삭제됨(AGENTS.md §6).
>      2) 상호 참조 깨짐: 이 문서에는 번호가 붙은 절이 없어 "아래 §6·§7"이 가리키는 대상이 없음.
>         실제 대상은 "원 결정 문서 (guide/)" 목록의 6·7번 항목이므로 항목 번호 표기로 교체 필요. -->
