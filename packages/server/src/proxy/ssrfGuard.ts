...
function isBlockedV6(ip: string): boolean {
  ...
  if (lower.startsWith("fc") || lower.startsWith("fd")) return true; // fc00::/7 ULA(fd00::/8 포함)
  return false;
}
...
export function isBlockedAddress(ip: string): boolean {
  ...
  if (net.isIPv4(unwrapped)) return isBlockedV4(unwrapped);
  if (net.isIPv6(unwrapped)) return isBlockedV6(unwrapped);
...
/** Node dns.lookup 콜백 시그니처 (all:false / all:true 양쪽). */
type LookupCallback =
  | ((err: NodeJS.ErrnoException | null, address: string, family: number) => void)
  | ((err: NodeJS.ErrnoException | null, addresses: ResolvedAddress[]) => void);
...
/**
 * `dns.lookup` 호환 훅. 연결 계층(node:http/https의 `lookup` 옵션, undici connect 등)에
 * 꽂으면 매 연결마다 resolve→검증을 다시 수행하고 hard-deny IP면 연결을 실패시킨다.
 * 이것이 "resolve된 IP로 고정"(§4.1)의 구현 — 검증한 바로 그 주소로만 소켓이 열린다.
 */
export function createGuardedLookup(resolver: Resolver = defaultResolver) {
  return function guardedLookup(
    ...
    resolver(hostname)
      .then((addresses) => {
        ...
        if (wantAll) {
          (callback as (e: null, a: ResolvedAddress[]) => void)(null, safe);
        } else {
          const first = safe[0]!;
      ...
      .catch(() => {
        const err = new Error(`SSRF: 호스트 해석 실패: ${hostname}`) as NodeJS.ErrnoException;
        err.code = "ENOTFOUND";
        (callback as (e: NodeJS.ErrnoException) => void)(err);
      });
  };
