...
const GUIDE_BODY = `
    ...
    <section class="g-section">
      ...
      <ul class="g-list">
        ...
        <li>화면은 한 번에 하나씩 보이고, <b>화면영역 아래의 [← 이전] [다음 →]</b>로 옮깁니다. 가운데 <code>2 / 3</code>이 지금 몇 번째인지 알려 줍니다.</li>
        <li>화면 이동이 지정돼 있으면 상단에 <b>프로세스 흐름도</b>가 그려집니다. 기본은 접혀 있고 펼치면 노드를 눌러 <b>건너뛰어 이동</b>할 수 있습니다.</li>
        <li>화면영역과 우측 어노테이션의 2단 구성이며 각 영역이 따로 스크롤됩니다. <b>스냅샷은 화면영역에 맞게 줄여</b> 화면 하나가 한눈에 들어옵니다.</li>
      </ul>
...
${faqGroup("편집", [
  faqItem(
    "앱에서 다른 화면으로 이동했는데 이전 화면의 마커가 그대로 보입니다.",
    `<p>의도된 동작입니다. 현재 화면은 주소가 아니라 <b>패널에서 선택한 화면</b>이므로, 앱에서 화면을 옮겨도 자동으로 바뀌지 않습니다.</p>
           <p>SPA처럼 주소가 바뀌는 이동이면 <b>새 화면으로 등록할까요?</b> 배너가 등록을 제안합니다. 주소가 바뀌지 않는 탭·MDI 화면은 패널에서 대응하는 화면을 직접 선택하거나 새로 등록하세요.</p>`
  ),
])}
