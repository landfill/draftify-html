...
연결된 사이트 위에 편집 패널이 뜬다 — 보던 화면 그대로 **[+ 현재 화면 등록]**(동결)하고, 편집 모드에서 요소를 클릭해 어노테이션을 단다. 화면 이동(예: 조회 → 검색 결과)도 지정할 수 있다. 저장은 자동.

![로그인된 실제 화면 위에서 화면 등록과 어노테이션 편집](./docs/images/pathD-3-live-edit.png)

### 4. 산출물 예시

**[내보내기 (HTML 다운로드)]** 결과물 — 프로세스 흐름도, 화면 네비, 마커가 정렬된 동결 스냅샷, 어노테이션 패널이 담긴 **단독 HTML 하나**다. `file://`로 열리고 네트워크 요청 0건.
