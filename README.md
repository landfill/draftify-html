...
- **편집은 라이브, 산출물은 동결** — 동결은 사용자 브라우저에서 일어나고, 결과물은 서버 없이 재현된다
...
| **D. 내 화면에서 편집** | 로그인해야 보이는 화면·사내망·로컬 (브라우저 확장) | [가이드 §3](./docs/user-guide.md#3-경로-d--내-화면에서-편집-확장--상세) |

이 README는 **경로 A(ZIP 업로드)의 빠른 시작**을 다룬다.
...
| 하려는 것 | 방법 |
|-----------|------|
| **화면 등록** | 목업을 원하는 상태로 조작(라우팅·모달·필터…) → **[+ 현재 화면 등록]** → `✓ 동결됨` 배지 확인 |
