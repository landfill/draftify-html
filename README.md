...
> **아래 화면은 전부 사내판**(`packages/server`, `localhost:4000`) 기준이다 — 이 README의
> 기본 배포(위 "배포가 둘이다" 표). 공개판은 헤더·콘솔 레이아웃이 다르다.
> 목업으로는 콘솔 [샘플 보기]와 같은 데모 화면(그린마트 주문 관리)을 썼다.
...
- 한 줄에 표시되는 것: 등록 방식 배지 · (확장은) 프로젝트 ID · 작성자 · 화면·어노테이션 수 · 내보내기 이력
- 확장 프로젝트의 연결 코드 재발급:
...
- 연결된 사이트 위에 편집 패널 표시
- 보던 화면 그대로 **[+ 현재 화면 등록]** → 화면이 `SCR-00N`으로 등록되고 그 자리에서 **캡처**된다
- 편집 모드에서 요소를 클릭하면 마커가 붙고, 오른쪽에 어노테이션 카드가 열린다
- 화면 이동(예: 로그인 → 주문 목록)은 어노테이션의 **이동 대상** 선택으로 지정
- **페이지 헤더**(섹션 라벨·페이지 타이틀)는 산출물 상단 밴드에 실린다 — 비우면 표시하지 않는다
- 저장은 자동
...
- 구성: 상단 제목 밴드(작성자·생성일·화면/어노테이션 수) · 접었다 펼치는 **프로세스 흐름도** ·
  **캡처 스냅샷 + 어노테이션 패널의 2컬럼** · 하단 화면 페이저(`← 이전 · N/M · 다음 →`)
- 흐름도의 현재 화면은 강조되고, 어노테이션의 이동 대상은 **[→ … 보기]** 링크로 따라갈 수 있다
- `file://`로 열리고 네트워크 요청 0건

![산출물 HTML — 흐름도·캡처·어노테이션 2컬럼·화면 페이저](./docs/images/pathD-4-export-viewer.png)
