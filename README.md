...
> 각자가 이미 만든 목업(빌드 산출물 zip)을 등록하면, 그 위에 숫자 어노테이션을 달아
> **단독 실행 가능한 기획서 HTML**을 만들어 주는 사내 서비스.
> 산출물은 `file://`로 열리고 네트워크 요청이 0건이다.

이 문서는 **서비스를 실행하고 사용하는 방법**을 다룬다.
...
| 동작 | 방법 |
|------|------|
| 장면 등록 | 목업을 원하는 상태로 조작(라우팅·모달·필터 등) → **[+ 현재 화면을 장면으로]** → `✓ 동결됨` 배지 확인. 실패 시 배지에서 재시도 |
...
장면은 "URL"이 아니라 **사람이 의미를 부여한 화면 상태**다. 화면 상태마다
"장면 등록 → 어노테이션 부착"을 반복한다. 어노테이션은 항상 현재 선택된 장면에 소속된다.

## 4. 내보내기 → 산출물 확인

1. 콘솔 목록에서 **내보내기** 클릭 → 단일 HTML 다운로드 (스냅샷 없는 장면이 있으면 확인 후 진행)
2. 다운로드한 HTML을 더블클릭(`file://`)으로 연다 — 서버·네트워크 없이 동작해야 정상:
   - 장면 전환이 되고 각 장면이 편집 때 모습과 동일
